...
- docs: 文言棚卸し Excel に Claude の修正案を記入(オーナー依頼 2026-09-19)。533 行のうち 141 行に案を入れ(修正 131 / 削除 7 / 保留 3)、残りは「このままでよい」。方針は ①分量削減 ②平易化 ③**保護者が見る画面から「開発者」「管理者」等の社内用語を排除**(問い合わせ先は「教室」に統一)。列を J 修正案 / K 対応 / L 変更理由 / M オーナーメモ に変更 (docs/ui-text-review-2026-09-19.xlsx)

- docs: 室長・保護者が見る画面の文言を棚卸しした一覧を追加(オーナー依頼 2026-09-18・文言の修正はオーナーが行う)。画面ごとに文章・出る場所・`ファイル:行`をまとめ、開発用語の混入/表記ゆれ/重複を §E に整理 (docs/ui-text-review-2026-09-18.md)
