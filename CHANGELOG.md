...
- feat: **通知済み(notifiedAt)を外して室長 PC で再通知させる手動ツール**を追加(坂口講師の提出が開発者画面で「通知済み」になった件の復旧用)。`tools/lecture-submission-notified-reset.mjs`(一覧 → `--token --apply` で 1 件だけ notifiedAt を削除。教室不一致・notifiedAt 無しは中止・提出内容は触らない=INV-07/INV-08)と GitHub Actions「Reset QR submission notified flag」(オーナーが実行。Claude は本番書き込みをしない)。一覧に「外せば再通知されるか」(提出が教室の前回保存より後か=App の起動時判定と同じ)を出す。純関数テスト 13 件 (tools/lecture-submission-notified-reset.mjs / .test.mjs / .github/workflows/reset-submission-notified.yml)
...
- chore(checklist): 確認リストを第24版へ。開発ダッシュボードの確認 d-1 を追加(結果待ちの b-2/b-3/c-2/t-1/s-4/q-1〜q-6 はそのまま)。**版は v1.5.556 据え置き**(項目を足しただけ。上げると結果待ち項目の下書きと送信済みの結果が消える・regression-reviewer 指摘) (verificationChecklist.ts / verificationChecklist.test.ts)
...
- fix: 盤面で通常授業 A を別日 B へ振替し、その振替コマをさらに C へ動かしたとき、A に残る「休)」の日付が B のまま C に追いつかない不具合を修正(確認リスト v1.5.555 その他欄「元の授業の振替先日が追いついていません。日程表は問題ない」)。真因= A の移動元記録(moved)が持つ移動先日付は移動時に 1 度だけ作られ B→C の再移動で書き換わらないのに、盤面は**その記録の日付だけ**を出していた(生徒日程表はリンク解決=今その授業が置かれているコマを先に見るので正しかった)。盤面と配布用盤面を生徒日程表と同じ順「リンク先 → 自分の移動先」に揃えた(`resolveMovedSourceDestinationLabel` を唯一の規則として共有)。**移動先日付を持たない古い moved 記録はリンク先があっても空のまま**(2026-09-16 の回帰防止を維持・機能フラグ OFF の教室で表示を変えない)。記録・在庫会計は無変更(INV-06 不触・表示のみ)。INV-04 準拠(盤面／配布用盤面／日程表の内容一致)。回帰テスト 9 件(修正なしで 4 件落ちることを確認) (lessonLinks.ts / lessonLinks.test.ts / BoardShareScreen.tsx / boardShareVisibleDateLabel.test.ts)
- feat: 室長の「サーバーバックアップから復元(直近3日)」を**全教室へ昇格**(フラグ `managerSelfRestore` development-only → all-classrooms・確認リスト s-1〜s-3 OK・オーナー指示 2026-09-22「機能問題ないので本番へ展開して」)。functions / Firestore ルールは無変更(元から担当教室のみ許可・3 者一致ガードはそのまま) (featureRollout.ts / featureRollout.test.ts / docs/spec-save-restore.md §4-1)
- style: 開発用教室の確認リストパネルの文字を**全部 2 倍**(15〜18px → 24〜36px)・幅 560 → 1120px・折りたたみボタン 32 → 64px(確認リスト c-2 要改善「すべての文字サイズが今の倍になるように」) (App.css)
- chore(checklist): 確認リストを第23版(v1.5.556)へ。OK 済みの r-2/r-5/v-1/r-8/c-3/m-1〜m-3/s-1〜s-3 を外し、予行 y-2〜y-4 はオーナー判断で中止(「本番教室では以降の丸ごと振替だけ今の仕様なら問題ないので、この予行は不要」)につき外した。c-2 を 2 倍の確認へ差し替え、その他欄の修正確認 t-1(A→B→C の振替・盤面／配布用盤面／日程表)と s-4(本番教室にも復元パネルが出る・見るだけ)を追加。b-2/b-3・q-1〜q-6 は結果待ちのまま (verificationChecklist.ts / verificationChecklist.test.ts)

## v1.5.555 (2026-09-22)

- chore: 確認リスト(開発用教室の Web パネル)を最新化。版を v1.5.540 → v1.5.555 へ上げ(据え置きのままだと前回の OK/要改善 の印が残り、差し替えた b-2/r-2/r-5 が確認済みに見えていた)、OK 済みの y-1 を外し、前回その他欄の「最新データ表示なのに未保存」修正(v1.5.553)の確認 v-1 を追加。アプリの動作変更なし (verificationChecklist.ts / verificationChecklist.test.ts)

## v1.5.554 (2026-09-21)

- fix(test): 退塾スイープの配線テストが CI(LF チェックアウト)でだけ落ちるのを修正。複数行マーカーに CRLF を直書きしていた(Windows の作業ツリーでは通る)。読み込んだソースを LF に正規化して照合する。アプリの動作変更なし (studentWithdrawSweep.test.ts)

## v1.5.553 (2026-09-21)

- feat: 退塾生徒の行でも生年月日だけは修正できる(案2・オーナー確定 2026-09-21)。生年月日の誤入力で卒業扱い=編集不可になった生徒を「削除して作り直す」以外で救う。直して卒業扱いでなくなったら、卒業で自動入力された退塾日(印つき)だけ印ごと外す。手入力の退塾日は外さない (withdrawGuard.ts applyLockedStudentBirthDateCorrection / BasicDataScreen.tsx)
- feat: 退塾の扱いを新仕様へ(オーナー確定 2026-09-20 夜・確認リスト b-2/b-3・INV-06/INV-03/INV-02/INV-08)。①**退塾日を日付入力で直接入れた場合も、退塾ボタンと同じ「今日以降の盤面の痕跡消し」が確認ダイアログなしで走る**。「退塾ボタンが一過性コマンド(キュー)を出す」方式を撤去し、**盤面が『退塾日を過ぎているのに消去開始日(= max(退塾日, 今日[JST]))以降に痕跡が残る生徒』を検出して掃除する**方式へ一般化した(検出は純関数 collectStudentWithdrawSweepTargets。掃除は従来の computeStudentWithdrawSweep を対象者ぶんまとめて適用し commitWeeks は 1 回＝Undo も 1 回。対象 0 なら何もしない=開いただけで未保存にしない)。未来の退塾日はその日を過ぎてから最初に盤面を開いた時点で走り、過去に退塾済みの生徒に残っていた今日以降の痕跡も同じ仕組みで消える。発火点は盤面のマウント時と students 変更時。安全条件はテンプレ編集中は走らせない/教室切替直後の窓では走らせない(isEditingStateLoadedForActingClassroom = App の loadedEditingClassroomIdRef === actingClassroomId。加えて applyClassroomPayloadToState が名簿・盤面・boardMountKey を同じ更新バッチで差し替える不変条件を wiring テストで固定・INV-08)/生徒の特定は managedStudentId 優先で名簿に一意な名前のみ/冪等(掃除後は対象 0)。在庫は従来どおり未消化へ戻さず、消したことで湧く分だけ抑止へ積む(台帳・希望数・QR 提出は不変)。②**高3の卒業も退塾扱いにした(案A)**: 4/1 になったら卒業年度を過ぎた高3の withdrawDate へ 3/31 を**実データとして自動入力**する(1 人につき 1 回だけ＝印 graduationWithdrawAutoFilledAt・既存の退塾日は上書きしない・高3以外と生年月日なしは対象外)。★判定関数(isActiveOnDate / resolveManagedStudentRosterStatus / hasGraduatedHighSchool)は無改変で、**データを入れることで**請求の在籍数・盤面・日程表・退塾スイープに乗せる(共有判定のロックテストは維持)。③基本データ画面: 「非在籍生徒表示」→**「退塾生徒」**に改名(testid は不変)。**在籍中(退塾日が未来)は退塾予定日を自由に編集でき、退塾後(非在籍)の行は氏名・退塾日ふくめ一切編集できない(「編集」ボタンも出さない)＝残る操作は「削除」だけ**(退塾は今日以降の痕跡を消し切る＝元に戻せないため、戻せる見た目を作らない)。Excel 差分取り込みなど別経路でも退塾済みの行は変更しない(preserveWithdrawnStudentRowsOnImport。取り込み結果に無い退塾済みの行は落とさず残す)。退塾の確認モーダルに「退塾にすると元に戻せません」を追加し、削除の確認モーダルを「この生徒をデータ上から削除します。元に戻せません」へ改定(未消化残数の警告は維持)。 (studentWithdrawSweep.ts / graduationWithdraw.ts(新規) / withdrawGuard.ts / deleteGuard.ts / basicDataModel.ts StudentRow.graduationWithdrawAutoFilledAt / parentAbsenceTarget.ts resolveBoardStudentOwnerId / ScheduleBoardScreen.tsx / App.tsx / BasicDataScreen.tsx / docs/spec-basic-data.md §B・§H / docs/spec-invariants.md INV-02・INV-06・INV-10 / studentWithdrawSweep.test.ts 刷新 / graduationWithdraw.test.ts(新規 10 件) / withdrawGuard.test.ts +4 / deleteGuard.test.ts / inv02 マトリクス +2 / 確認リスト b-2 更新・b-3 追加(版 v1.5.540 据え置き))
  - chore(段階導入・レビュー指摘): **自動消去(盤面の退塾スイープ)と高3卒業の退塾日 自動入力は、開発用教室限定の機能フラグ `studentWithdrawAutoSweep` で先行**させる(昇格＝staging→本番はオーナー確認後)。どちらも室長の確認なしに実データ・盤面を書き換えて自動保存へ載る操作のため。フラグ OFF の教室(本番3教室)は従来どおり通常授業の剥がしだけ／表示上だけの 3/31 補完のまま。「退塾生徒」への改名・退塾後の行ロック・削除の確認文言・Excel 差分取り込みの退塾済み行ガードは**フラグに依らず全教室で有効**で、退塾確認モーダルの本文だけフラグで出し分ける(OFF で「消えます」と書かない)。**本番実データの事前実測(2026-09-21・バックアップ JSON に純関数を当てた読み取り専用)**: 本番3教室の初回の消去対象は 0 名・0 件、卒業自動入力の対象も 0 名で、最初の発火は 2027-04-01 (featureRollout.ts / App.tsx / BasicDataScreen.tsx withdrawGuard.ts buildStudentWithdrawConfirmation.autoSweepEnabled / featureRollout.test.ts / ScheduleBoardScreen.featureFlags.wiring.test.ts / docs/spec-basic-data.md §B)
  - fix(レビュー指摘・中): **掃除の入力を「再マージ適用後の最新 weeks」にした**(INV-02)。再マージ effect(deps に students・`setWeeks` の関数形で最新に適用)と掃除(`commitWeeks` へ明示値)は基本データから戻った同じ flush で走るため、掃除がその render の古い weeks から作った配列を commit すると、同じ flush の再マージ結果(例: 別の生徒の改名反映・通常授業の追加)が落ちた盤面がそのまま自動保存されていた。週ごとの再マージを合成関数 `remergeBoardWeeksWithManagedData` へ寄せ(読込・名簿変更 effect・掃除の 3 経路が同じ結果を使う)、掃除は純関数 `applyStudentWithdrawSweepToBoard` の中で先に再マージを通してから検出・掃除する(再マージは冪等なので掃除だけのときに余計な差分は出ない。何も消さなければ盤面を差し替えない)。820365d で消えたマウント時の実行順の注意書きも新実装に即して復活 (ScheduleBoardScreen.tsx / studentWithdrawSweep.test.ts +2)
  - fix(レビュー指摘・低): Excel 差分取り込みで一致行を置き換えるとき `graduationWithdrawAutoFilledAt` を引き継ぐ(保護者トークンの写しと同じ扱い。消えると「1 人 1 回だけ」のブレーキが外れる)。`resolveStudentWithdrawSweepFromDateKey` に「厳格書式以外は今日へ落とす・別用途で使わない」注記。テスト追加: 卒業の学年境界(現役高3・早生まれ高3・高2は入れない／卒業済みは 3/31・4月1日生まれは既存挙動を既知として固定)、退塾日の書式揺れ(未定/空/2026-13-40 は誤検出しない・スラッシュ書式は対象)、休日解除の兄弟行(m) 振無休×在庫由来の別日組み直し、`removeMakeupOrigin` の同日 時限つき/時限なし併存 (BasicDataScreen.tsx / graduationWithdraw.test.ts / studentWithdrawSweep.test.ts / inv06-holiday-record-retention.matrix.test.ts / inv06-makeup-absence-stock.matrix.test.ts)
- feat: 休日設定を解除したら、休み扱い(「休)」= holiday 記録)になっていた生徒を**全員元の席へ戻す**ようにした(休日設定の逆操作・確認リスト r-5 要改善・オーナー確定 2026-09-20・INV-06/INV-03/INV-12)。元の机・席・科目・講師で配置として戻し、休日設定で増えた未消化(振替 origin / 講習 +1)を巻き戻す。巻き戻しの根拠は休日設定時に記録へ焼き込んだ控え holidayStockReturn **だけ**にして、設定→解除の往復で台帳が完全一致するようにした(配置由来と出欠記録由来で設定時の会計が違い、記録から再導出すると誤増/誤減になる)。その未消化を既に別日へ組んでいた場合はその振替/講習コマを盤面から消し、件数を確認ダイアログと完了メッセージで知らせる(振替=コマ削除の消化 −1 と origin 除去で ±0／講習=設定の +1 と別日配置の −1 が打ち消し合うので台帳は触らない)。戻さないのは「控えの無い旧データ」「席が別の生徒で埋まっている」「別日のコマに出欠記録がある(実施済みは消さない)」の3つで、記録も台帳も触らず件数だけ知らせる。戻した通常授業は抑止(suppressedRegularLessonOccurrences)を積まない・既にあれば外す(積むと再マージで戻した席が落ちて消える)。absent/moved の記録と個別ボタン「休日記録の表示解除」の仕様は不変。フラグ OFF の教室・holiday 記録が無い日は従来どおり (ScheduleBoardScreen.tsx computeHolidayReleaseRestoration / reconcileHolidayDeskStockReturns stockReturnStamps / types.ts HolidayStockReturnStamp / docs/spec-makeup-stock.md §B-2-2b・§B-2-2c / docs/spec-invariants.md INV-06 / inv06-holiday-record-retention.matrix.test.ts +13 / holidayReleaseRestoration.wiring.test.ts)
  - fix(レビュー指摘・中): 控えが `kind:'none'` のときも「その未消化を別日に組んだコマ」を検査するようにした(INV-06)。`none` には**在庫由来の振替**(休日設定では台帳を積まず、配置が消えると台帳 origin が自動で再浮上するもの)が含まれ、再浮上した分を別日へ組み直してから解除すると**別日のコマが残ったまま元の席も復活して 1 origin に 2 コマ**になっていた。控えに origin が無いので記録自身の makeupSourceDate/makeupSourceLabel を origin として同じ検査にかけ、配置なら消して復元・出欠記録つきなら復元を見送る(在庫は中立=別日を消して再浮上・席を戻して再消化)。手動追加(manualAdded)と手動追加の講習は在庫を消化しておらず再浮上もしないので対象外。併せて台帳から origin を外すとき**積んだときと同じ形**(控えの時限 → 時限なし → 同日の先頭)を外すようにした(同じ日に時限つき/時限なしの origin が併存すると、先頭固定では件数は合うのに照合先が入れ替わり残数が狂う) (ScheduleBoardScreen.tsx computeHolidayReleaseRestoration / removeMakeupOrigin / docs/spec-makeup-stock.md §B-2-2c / inv06-holiday-record-retention.matrix.test.ts +4 行(i)〜(l))
- fix: 基本データの「退塾」ボタンで、その生徒の**今日以降**の盤面の痕跡(手で置いた講習・振替・増コマ・体験・手動追加・移動の席と出欠記録)も消し切るようにした(確認リスト b-2 要改善・オーナー確定 2026-09-20・INV-06/INV-03/INV-02/INV-01)。未消化振替・未消化講習へは戻さず(台帳へ +1 しない・「破棄前に確定」もしない)、消したことで在庫が湧く分だけ抑止(suppressedMakeupOrigins)へ積む(振替=振替元日／通常=元の通常授業日。absent/moved/holiday の記録には積まない=誤減防止)。希望数と QR 提出は不変。昨日以前・他の生徒・講師・メモは不変。基本データ→盤面は一過性コマンドのキューで渡し、盤面がマウントされた時点で 1 件だけ処理して commitWeeks を 1 回通す(再マージ/読込の経路には混ぜない・教室切替/復元/ログアウトでキューを捨てる=INV-08)。確認モーダルの案内と確認リスト b-2 も新仕様へ (studentWithdrawSweep.ts / ScheduleBoardScreen.tsx computeStudentWithdrawSweep / App.tsx / BasicDataScreen.tsx / withdrawGuard.ts / docs/spec-basic-data.md §B・§H / docs/spec-invariants.md / inv02・inv06 マトリクス +5)
