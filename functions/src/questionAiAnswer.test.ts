...
describe('questionAiAnswer: 質問への AI 即時回答(開発用教室のみ・spec-developer-report §G-7)', () => {
  it('AI を呼ぶのは 質問 × 開発用教室 × 確認リストでない ときだけ(本番教室・要望・不具合では呼ばない)', () => {
    ...
    for (const id of ['5w5OMueETerSKrSf14HC', 'KzFnOQoTFLsCxwUp1tvh', '6xnnbSTbwgGrBLy0EJKb']) {
      expect(shouldAnswerQuestionWithAi({ category: 'question', isDevelopmentClassroom: isDevelopmentClassroomIdentity({ workspaceKey: 'main', classroomId: id }), isVerificationChecklist: false })).toBe(false)
    }
    expect(shouldAnswerQuestionWithAi({ category: 'question', isDevelopmentClassroom: isDevelopmentClassroomIdentity({ workspaceKey: 'company-b', classroomId: 'v8OZ7zH8vONNHjjYVcR1' }), isVerificationChecklist: false })).toBe(false)
