...
describe('確認リストの項目定義', () => {
  ...
  it('第8版: 前回 OK だった項目は載せず、第7版で要改善だった k-4 の再確認だけ(オーナー指摘 2026-09-12 の運用)', () => {
    const ids = VERIFICATION_CHECKLIST.items.map((item) => item.id)
    expect(ids).toEqual(['k-4'])
    // 第7版で k-5(月の移動)は OK。
    ...
    expect(VERIFICATION_CHECKLIST.version).toBe('v1.5.519')
  })
})

describe('下書きの保存キーと往復', () => {
  it('教室別・版別のキーになる', () => {
    expect(verificationChecklistStorageKey('v8OZ7zH8vONNHjjYVcR1')).toBe('verification-checklist:v8OZ7zH8vONNHjjYVcR1:v1.5.519')
    expect(verificationChecklistStorageKey(null)).toBe('verification-checklist:unknown:v1.5.519')
...
describe('送信本文の書式', () => {
  it('先頭行が固定マーカー・未確認は省略・要改善はメモ付き', () => {
    ...
    expect(buildVerificationChecklistMarker()).toBe('[確認リスト v1.5.519]')
  })
