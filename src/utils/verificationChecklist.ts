...
export const VERIFICATION_CHECKLIST_VERSION = 'v1.5.504'

/**
 * 確認項目の正本(2026-09-12 初版・v1.5.504 で第2版)。
 * U-0(戻す/やり直しの保存) / 盤面PDFコマ選択 / 通常授業履歴 の3領域。
 * 第2版: v1.5.502 の確認リスト結果(要改善 p-1/p-2/p-3/p-6/h-1/h-2)への修正を確認する項目に差し替え・追加。
 */
export const VERIFICATION_CHECKLIST: VerificationChecklistDefinition = {
  ...
  items: [
    ...
    },
    {
      ...
      area: 'PDF',
      title: '全選択の出力が従来と同じ',
      steps: ['「PDF出力」でモーダルが出る', '全選択にして出力する', '従来と同じ PDF になる(集団行の「集団」だけ横書きに変わる)'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'p-2',
      ...
      steps: ['曜日を1〜2日に絞って出力する', '生徒・講師名・席番号が揃って大きくなる', '太りすぎ・崩れ・見切れがない'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'p-3',
      area: 'PDF',
      title: '時限を絞ると行が伸びて A3 縦いっぱいになり、3文字の講師名も見切れない',
      steps: ['3コマ×全曜日で出力する', '下半分が白紙にならず行が縦に伸びている', '3文字の講師名が最後まで見える', '時限ラベルが対応する行に付いている'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'p-8',
      area: 'PDF',
      title: '日曜(定休日)も選べる',
      steps: ['モーダルに日曜(定休日)の列が薄く出る', '日曜だけ選んで出力できる'],
      introducedIn: 'v1.5.504',
    },
    {
      id: 'p-9',
      area: 'PDF',
      title: '前回の選択が教室ごとに記憶される',
      steps: ['一部だけ選んで出力する', '別の週に移動して「PDF出力」を開く', '同じ曜日×時限が選ばれた状態で開く', '別教室では別の記憶になっている'],
      introducedIn: 'v1.5.504',
    },
    {
      id: 'p-10',
      area: 'PDF',
      title: 'ファイル名が選んだ曜日・時限になる',
      steps: ['曜日と時限を絞って出力する', 'ファイル名が「予定表9月14日(月)-9月16日(水) 1限-3限」のようになる', '全選択は従来の「週間予定表…」のまま'],
      introducedIn: 'v1.5.504',
    },
    {
      id: 'p-4',
    ...
    },
    {
      ...
      steps: ['集団授業2行がある週で列を絞って出力する', '集団授業の行が崩れない', '左端の「集団」が横書きで全部見える'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'p-7',
    ...
    },
    {
      id: 'h-1',
      area: '日程表',
      title: '「通常授業履歴」ボタンの名前と位置',
      steps: ['生徒日程表タブで「講習集計結果」の左に「通常授業履歴」が出る(旧「講習履歴」)', '講師日程表には出ない'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'h-2',
      area: '日程表',
      title: '既定期間(1年)の表と集計が出る・「表示」で入力欄が書き換わらない',
      steps: ['生徒を選んで「通常授業履歴」を開く', '既定期間(1年)の表と集計が出る', '開始日を変えて「表示」を押しても開始日・終了日が勝手に書き換わらない', '開始日を終了日より後にして「表示」を押すと理由が出て入力欄はそのまま', '注記に「表示期間」が出る'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'h-3',
      ...
      steps: ['期間を2年にする', '「1年分に丸めました」相当の注記と丸め後の「表示期間」が出る(入力欄は書き換わらない)'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'h-4',
