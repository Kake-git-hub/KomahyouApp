...
export const VERIFICATION_CHECKLIST_VERSION = 'v1.5.518'
  ...
  items: [
    {
      id: 'k-4',
      area: '保護者QR',
      title: '保護者ページが1か月単位で、授業のある日と臨時休みの日だけ出る',
      steps: ['QR をスマホで読む(または /p/ の URL をスマホで開く)', '期間の見出しが「2026年9月」のような月表示で、今月の分が出ている', '授業(通常・振替・お休み)がある日だけが並び、授業の無い日や毎週の定休日(日曜など)の行は出ない', '基本データで休校日にした日(臨時・祝日休み)は「教室休み」の1行が出る', '講習期間の日も「講習期間(別途ご案内)」にならず、通常授業がそのまま出る(講習のコマは出ない)'],
      introducedIn: 'v1.5.518',
    },
    {
      id: 'k-5',
      area: '保護者QR',
      title: '「前の月」「次の月」で先月・来月まで動かせる',
      steps: ['「次の月」を押すと来月の分に変わる', '来月でさらに「次の月」は押せない', '「前の月」を2回押すと先月の分になり、先月でさらに「前の月」は押せない'],
      introducedIn: 'v1.5.518',
    },
  ],
}
