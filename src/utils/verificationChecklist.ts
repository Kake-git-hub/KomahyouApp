...
export const VERIFICATION_CHECKLIST: VerificationChecklistDefinition = {
  ...
  items: [
    ...
    },
    {
      ...
      prep: '今日の盤面に通常授業がある生徒を1人選び、その生徒の講習か振替のコマを今日か明日へ1つ手で置き、未消化振替/未消化講習の残数を控える',
      steps: [
        '基本データ → 生徒タブ → その生徒の「退塾」→ 確認画面(「今日以降の講習・振替などのコマと記録も消えます」)→「退塾にする」',
    ...
    },
    {
      ...
      prep: 'r-3 で休日設定した日で行う。未消化振替・未消化講習の残数を控え、うち1人の振替だけ別の日へ組んでおく',
      steps: [
