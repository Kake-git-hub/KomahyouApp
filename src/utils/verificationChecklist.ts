...
export const VERIFICATION_CHECKLIST_VERSION = 'v1.5.519'
  ...
  items: [
    {
      id: 'k-4',
      area: '保護者QR',
      title: '講習だけだった月は「通常授業がありません」の注記が出る',
      steps: ['8月が夏期講習だけだった生徒(例: 8/26 入塾の生徒)の QR をスマホで開き、「前の月」で 8 月にする', '休校日の「教室休み」の行の下に「この月は通常授業がありません（講習の日程はこのページには表示されません）。」が出る', '8月に通常授業があった生徒の QR では、8 月の出席(「出席」)の行が並び、この注記は出ない', '授業も講習も無い月(例: 入塾前の月)は今までどおり(注記は出ない)'],
      introducedIn: 'v1.5.519',
    },
  ],
}
