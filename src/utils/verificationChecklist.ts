...
export const VERIFICATION_CHECKLIST_VERSION = 'v1.5.506'

/**
 * 確認項目の正本(2026-09-12 初版・v1.5.504 で第2版・v1.5.506 で第3版)。
  ...
  items: [
    {
      id: 'c-1',
      area: '確認リスト',
      title: '前回 OK だった項目が残っていない',
      steps: ['このリストに、前回 OK を付けた項目(戻す/やり直し・定休日選択・記憶・ファイル名など)が出ていない', '出ているのは再確認 3 件と新規 3 件だけ'],
      introducedIn: 'v1.5.506',
    },
    {
      id: 'p-2',
      area: 'PDF',
      title: '曜日を絞っても行の高さが揃う(生徒のいる行だけ高くならない)',
      steps: ['曜日を1〜2日に絞って出力する', '生徒のいる行と空席の行が同じ高さになっている', '生徒名・学年科目が行に収まって見切れていない', 'A3 縦いっぱいに広がっている'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'p-3',
      area: 'PDF',
      title: '時限を絞っても行の高さが揃う',
      steps: ['3コマ×全曜日で出力する', '生徒のいる行と空席の行が同じ高さになっている', '講師名・生徒名が見切れていない'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'p-11',
      area: 'PDF',
      title: 'PDF出力中は画面全体にスピナーが出る',
      steps: ['「PDF出力」→「出力」を押す', '出力が終わるまで画面全体に「PDF出力中…」のスピナーが出て、下の盤面を操作できない', 'PDF が保存されるとスピナーが消える', 'テンプレ上書き保存のレポート PDF でも同じスピナーが出る'],
      introducedIn: 'v1.5.506',
    },
    {
      id: 'h-2',
      area: '日程表',
      title: '通常授業履歴が表示される(INTERNAL にならない)',
      steps: ['生徒日程表タブで生徒を選び「通常授業履歴」を開く', '既定期間(1年)の表と集計が出る(保存済みの記録のみ)', 'もしエラーが出たら、その文言をそのままメモに書く(原因が文言に出るようにした)'],
      introducedIn: 'v1.5.502',
    },
    {
      id: 'h-8',
      area: '日程表',
      title: '履歴の読み込み中にスピナーが出る',
      steps: ['「表示」を押す', '結果が出るまで「読み込み中」のスピナーが出る', '結果が出るとスピナーが消える'],
      introducedIn: 'v1.5.506',
    },
  ],
}
