...
export const VERIFICATION_CHECKLIST: VerificationChecklistDefinition = {
  ...
  items: [
    ...
    // 室長の自教室復元(docs/spec-save-restore.md §4-1・フラグ managerSelfRestore・開発用教室限定)。
    {
      id: 's-1',
      area: 'バックアップ/復元',
      title: '「サーバーバックアップから復元(直近7日)」で、違うパスワードは弾かれる',
      steps: [
        'メニュー → バックアップ/復元/初期設定 →「バックアップ時点を取得」→ 時点を1つ選んで「この時点へ復元」',
        'わざと違うパスワードを入れて「この時点を読み込む」',
      ],
      check: [
        '時点の一覧が直近7日分だけ・新しい順に並ぶ(直近24時間は15分毎)',
        '注意書きに「この教室だけ」「保存していない編集は失われます」「復元の対象外」が出ている',
        '「ログインパスワードが一致しません。」と出て、モーダルが閉じず、盤面のデータは変わらない',
      ],
    ...
    },
    {
      id: 's-2',
      area: 'バックアップ/復元',
      title: '正しいパスワードで過去の時点を読み込める。保存前なら「直前の状態に戻す」で取り消せる',
      prep: '保存はしない(開発用教室には y 系の確認用に日大前データを入れてあるため、読み込んだら必ず取り消す)',
      steps: [
        '同じ画面で、できるだけ古い時点を選び「この時点へ復元」→ 正しいログインパスワード → 確認画面の人数を見て OK',
        '盤面に切り替わったら内容を見て、上部の「直前の状態に戻す」を押す',
      ],
      check: [
        '確認画面に「開発用教室」の名前と、生徒数/講師数/テンプレのコマ数が出る',
    ...
    },
    {
      ...
      check: [
        '「サーバーバックアップから復元(直近7日)」のパネルが無い(手動バックアップ・復元・初期設定フローだけ)',
      ],
      introducedIn: 'v1.5.548',
    },
  ],
