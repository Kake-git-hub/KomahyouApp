...
export const VERIFICATION_CHECKLIST_VERSION = 'v1.5.521'
  ...
  items: [
    {
      id: 'k-10',
      area: '保護者QR',
      title: '日程は1コマ1行で並び、スクロールが短くなった',
      steps: ['保護者用QRをスマホで開く', '1コマが1行(日付・時限と開始時刻・科目)で並び、同じ日の2コマ目以降は日付欄が空いている', '教室休みの日は1行で「教室休み」', 'テンプレ由来の授業は行末に小さな「予定」印、一覧の上に意味の説明が1行出る'],
      introducedIn: 'v1.5.521',
    },
    {
      id: 'k-11',
      area: '保護者QR',
      title: '休みの行に振替先、振替の行に振替元が「月日 コマ」で出る',
      steps: ['振替を割り振り済みの生徒の保護者用QRを開く', '休みの行に「振替先: 9月20日 2限」のように出る', '振替の行に「振替（振替元: 9月15日 4限）」のように出る', '振替を出席にした行でも「出席済み（振替元: …）」と出る'],
      introducedIn: 'v1.5.521',
    },
    {
      id: 'k-12',
      area: '保護者QR',
      title: '生徒を追加した直後は、保存が終わるまで QR ボタンがスピナーになる',
      steps: ['基本データで生徒を追加する', 'その生徒の QR ボタンが回るマーク付きで押せない状態になる', '保存が終わると普通の QR ボタンに戻り、押すと発行できる(発行失敗にならない)', '既存の生徒の QR ボタンは追加・編集中もそのまま押せる'],
      introducedIn: 'v1.5.521',
    },
    {
      id: 'h-9',
      area: '日程表',
      title: '通常授業履歴の期間が効き、既定は今月の1日〜末日',
      steps: ['生徒日程表で生徒を選び「通常授業履歴」を開く', '開始日・終了日が今月の1日〜末日になっていて、今月の記録だけが出る', '開始日・終了日を変えると「表示」を押さなくても、その期間の記録に切り替わる(注記の「表示期間」も変わる)', '記録を付け始める前の月(例: 6月)を指定しても、その月の記録が出る(0件にならない)'],
      introducedIn: 'v1.5.521',
    },
    {
      id: 'b-1',
      area: '盤面',
      title: '「通常授業テンプレ作成」ボタンが「未消化講習」の左にある',
      steps: ['コマ表(盤面)を開く', '右上のボタン列で「通常授業テンプレ作成」が「未消化講習」のすぐ左に並んでいる', '押すと今までどおりテンプレ作成に入れる'],
      introducedIn: 'v1.5.521',
    },
  ],
}
