...
export const VERIFICATION_CHECKLIST: VerificationChecklistDefinition = {
  ...
  items: [
    ...
    // 緑が丘 室長指摘(2026-09-26・Issue #70 のやり取り)「講師日程共有の画面で振替先日付が追従していない」の修正確認。
    // 共有するのは今週以降の週だけなので、振替先を**今週より前の週**へ動かし直したときだけ起きていた(t-1 は今週内の B・C)。
    {
      id: 't-2',
      area: '配布用盤面',
      title: '振替を今週より前の週へ動かし直しても、講師日程共有の「休」の日付が盤面と同じ(最後の移動先)になる',
      prep: '今週〜来週の通常授業(通)を 1 人選ぶ(A)。来週の空きコマ(B)と、先週の空きコマ(C)を決めておく',
      steps: [
        'A の生徒名を長押し → B へドラッグ(A に「休 B の日付」が残る)',
        'B に置かれた振)を長押し → 先週の C へドラッグ → 保存 →「講師日程共有」の配布用URLを開き、A の日付・コマを選ぶ',
      ],
      check: [
        '盤面: A の「休」の日付が C の日付',
        '配布用盤面: A の「休」の日付も C の日付(前回は B のまま止まっていた)',
        '後片付け: C の振)を元に戻すか削除して保存する',
      ],
      introducedIn: 'v1.5.558',
    },
    ...
    },
    {
      id: 'd-2',
      area: '開発者画面',
      title: '開発者画面(教室運営管理)にいる間は QR 提出通知が出ず、開発用教室を開いたときに改めて出る',
      prep: '開発用教室の講習で講師 1 人の QR(日程表の提出用)をスマホで開けるようにしておく。PC は開発者でログインし、開発用教室を開いてから「ログアウト」で開発者画面に戻った状態',
      steps: [
        'スマホで講師の QR を開いて提出する → PC の開発者画面を 1 分ほど見る',
        'PC で開発用教室を開く',
      ],
      check: [
        '開発者画面には「QR提出通知」のモーダルが出ない(前回は講師名つきで出ていた)',
        '開発用教室を開いた直後に「QR提出通知」が出て、講師名と講習名が正しい',
        '「すべて確認」で閉じたあと、PC をリロードしても同じ通知は出ない',
        '講習集計結果でその講師が「提出済」になっている(提出データは通知の有無と関係なく反映される)',
      ],
      introducedIn: 'v1.5.559',
    },
    {
      id: 'd-3',
      area: '開発者画面',
      title: '開発者が本番教室を開いて QR 提出通知を見ても、その教室の室長 PC では次回起動でちゃんと通知が出る(開発者は表示だけ・通知済みを記録しない)',
      prep: '★本番教室では見るだけ(保存しない)。講師か生徒の QR 提出が届いた直後に、室長 PC が閉じている教室で試す',
      steps: [
        '開発者でログイン → その教室を開く → 出た「QR提出通知」を「すべて確認」で閉じる(保存はしない)',
        'その教室の室長 PC(または室長アカウント)でアプリをハードリロードする',
      ],
      check: [
        '室長側でも同じ「QR提出通知」が出る(前回は開発者が先に見ると室長側に出なかった)',
        '開発者側で教室を開き直すと同じ通知がもう一度出る(開発者は通知済みを記録しないため・仕様)',
        '開発用教室では従来どおり、一度「すべて確認」すればリロードしても出ない',
      ],
      introducedIn: 'v1.5.560',
    },
  ],
}
