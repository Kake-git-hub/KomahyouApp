...
function createBasePayload(params: OpenScheduleHtmlParams, linkedStudents: StudentRow[] = []): Omit<SchedulePayload, 'students' | 'teachers'> {
  ...
  return {
    ...
    questionAiAnswerEnabled: isFeatureEnabledForClassroom('questionAiAnswer', { id: params.classroomStorageKey }),
  }
}
...
export function buildSerializedScheduleCountAdjustments(params: {
  ...
  //
  // ★履歴（巻き戻し注意）: この special 限定ガードは `7ac998d`(v1.5.50「keep regular desired counts
  //   contract-based」) が意図的に置いたもの。同コミットの他の3点（削除調整 −1 を予定側に効かせない／
  //   予定側から regularCountAdjustments を外す／その回帰テスト）は `cc5e5e8`(2026-05-29 の寄せ集め
  //   コミット)で**無言のうちに巻き戻り**、テストも消えている。増コマへの +1 はその残りを 2026-08-05 の
  //   オーナー確定で**意図的に**上書きしたもの（増コマはテンプレ契約に存在しないコマなので
  //   contract-based の趣旨と衝突しない）。「契約ベースだから」を根拠に extra を戻さないこと。
  //
  // status 軸は実績側（生徒日程表の回数表）と**厳密に対称**にする（lessonType 軸は上記のとおり意図的に非対称）:
