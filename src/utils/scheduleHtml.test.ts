...
describe('日程表コマ組み 埋め込みJS: 掴めるカードのゲートと配線', () => {
  ...
  it('D&D・机選択・移動要求送信・再描画時破棄の配線が埋め込みJSに含まれる(削除の回帰検知)', () => {
    ...
    // 机選択モーダルは盤面の1コマを切り取った表形式(1机=1行)。
    expect(html).toContain('class="desk-picker-board"')
    expect(html).toContain('class="dp-seatno"')
    expect(html).toContain('class="dp-teacher"')
  })

  // 生成HTMLから renderDeskPickerSeatCellHtml(deskIndex, seat) を取り出し、席セルの形式を固定する。
  function extractSeatCellFn(html: string) {
    const match = html.match(/function renderDeskPickerSeatCellHtml\(deskIndex, seat\) \{([\s\S]*?)\n {6}\}/)
    expect(match).not.toBeNull()
    const fn = new Function('deskIndex', 'seat', 'escapeHtml', match![1]) as (deskIndex: number, seat: unknown, esc: (value: unknown) => string) => string
    const esc = (value: unknown) => String(value == null ? '' : value)
    return (deskIndex: number, seat: unknown) => fn(deskIndex, seat, esc)
  }

  it('机選択モーダルの席セルは盤面と同じ td 形式(空席=クリック可td・占有=非クリックtd)', () => {
    const run = extractSeatCellFn(openDndHtml({ scheduleDndEnabled: true }))
    const selectable = run(2, { studentIndex: 1, selectable: true, occupied: false })
    ...
    const occupied = run(0, { studentIndex: 0, occupied: true, selectable: false, label: '山田 数' })
    ...
    expect(run(1, { studentIndex: 0, blockedByMemo: true, selectable: false })).toContain('dp-blocked')
  })
})
