...
function createSpecialSessionAvailabilityHtml(payload: PopupPayload) {
  ...
  const serializedSubjects = JSON.stringify(['英', '数', '算', '算国', '国', '理', '生', '物', '化', '社']).replace(/</g, '\u003c')
...
<html lang="ja">
  ...
  <body>
    ...
    <script>
      ...
      function getPreferredMathSubject(student, referenceDate) { const gradeLabel = getGradeLabel(student && student.birthDate, referenceDate); return gradeLabel.startsWith('小') ? '算' : '数'; }
      function getVisibleSubjectsForStudent(student, referenceDate, additionalSubjects) { const preferredMathSubject = getPreferredMathSubject(student, referenceDate); const gradeLabel = getGradeLabel(student && student.birthDate, referenceDate); const prefersHighSchoolScience = gradeLabel.startsWith('高'); const legacySubjects = Array.isArray(additionalSubjects) ? additionalSubjects : []; return subjectDefinitions.filter((subject) => { if (subject === '算' || subject === '数') return subject === preferredMathSubject; if (subject === '算国') return preferredMathSubject === '算'; if (subject === '理') return !prefersHighSchoolScience || legacySubjects.includes('理'); if (subject === '生' || subject === '物' || subject === '化') return prefersHighSchoolScience || legacySubjects.includes(subject); return true; }); }
      function normalizeSubjectForStudent(subject, student, referenceDate) { if (subject === '算国') return getPreferredMathSubject(student, referenceDate) === '算' ? '算国' : '数'; if (subject !== '算' && subject !== '数') return subject; return getPreferredMathSubject(student, referenceDate); }
      function normalizeSubjectSlotsForStudent(subjectSlots, student, referenceDate) { const next = {}; Object.entries(normalizeSubjectSlots(subjectSlots)).forEach(([subject, value]) => { const normalizedSubject = normalizeSubjectForStudent(subject, student, referenceDate); next[normalizedSubject] = (next[normalizedSubject] || 0) + value; }); return next; }
      ...
      function buildLectureCountRows(student) { const input = getStudentInput(student); const normalizedSubjectSlots = normalizeSubjectSlotsForStudent(input.subjectSlots, student, state.startDate); const visibleSubjects = getVisibleSubjectsForStudent(student, state.startDate, Object.keys(normalizedSubjectSlots)); const subjectRows = visibleSubjects.map((subject) => '<tr><td>' + escapeHtml(subject) + '</td><td><input type="number" min="0" step="1" data-role="lecture-count-subject" data-student-id="' + student.id + '" data-subject="' + subject + '" data-testid="special-session-popup-subject-' + student.id + '-' + subject + '" value="' + escapeHtml(String(Number(normalizedSubjectSlots[subject] || 0))) + '"' + (input.regularOnly ? ' disabled' : '') + '></td></tr>').join(''); const regularOnlyRow = '<tr><td>通常のみ</td><td><label class="count-check"><input type="checkbox" data-role="lecture-count-regular-only" data-student-id="' + student.id + '" data-testid="special-session-popup-regular-only-' + student.id + '"' + (input.regularOnly ? ' checked' : '') + '>通常のみ</label></td></tr>'; return subjectRows + regularOnlyRow; }
