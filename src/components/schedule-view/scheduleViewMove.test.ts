...
import {
  ...
  resolveScheduleViewTargetDeskIndex,
...
describe('resolveScheduleViewTargetDeskIndex: 机同一性での解決(positional のズレに強くする)', () => {
  // 佐藤(席0占有・席1空) / 鈴木(空机) の2机コマ。
  function makeTwoDeskCell(): SlotCell {
    return makeCell('2026-07-08', 3, [
      makeLessonDesk('佐藤', [makeStudentEntry({ id: 'occ' }), null]),
      { teacher: '鈴木' },
    ])
  }

  it('deskId 一致を最優先(positional がズレていても実机へ解決)', () => {
    const cell = makeTwoDeskCell()
    const targetDeskId = cell.desks[1].id
    // deskIndex=0(佐藤席0は占有)でも deskId で鈴木机(index 1・空)に解決される。
    expect(resolveScheduleViewTargetDeskIndex(cell, {
      targetDateKey: '2026-07-08', targetSlotNumber: 3, deskIndex: 0, studentIndex: 0,
      deskId: targetDeskId, deskTeacher: '鈴木',
    })).toBe(1)
  })

  it('deskId 未指定・deskIndex が範囲外でも講師名一致の空席机へ解決', () => {
    const cell = makeTwoDeskCell()
    expect(resolveScheduleViewTargetDeskIndex(cell, {
      targetDateKey: '2026-07-08', targetSlotNumber: 3, deskIndex: 9, studentIndex: 0,
      deskTeacher: '鈴木',
    })).toBe(1)
  })

  it('どれも一致しなければ元の deskIndex を返す(validate が最終的に弾く)', () => {
    const cell = makeTwoDeskCell()
    expect(resolveScheduleViewTargetDeskIndex(cell, {
      targetDateKey: '2026-07-08', targetSlotNumber: 3, deskIndex: 0, studentIndex: 0,
      deskId: 'nonexistent', deskTeacher: '存在しない講師',
    })).toBe(0)
  })
})
