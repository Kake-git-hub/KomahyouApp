...
type BackupRestoreScreenProps = {
  ...
  managerSelfRestoreEnabled?: boolean
              ...
              <section className="basic-data-section-card" data-testid="backup-restore-self-restore-panel">
                ...
                <div className="basic-data-form-grid">
                  <button
                    ...
                    type="button"
                    onClick={() => onLoadManagerSelfRestoreCandidates?.()}
                    disabled={managerSelfRestoreLoading}
                    data-testid="backup-restore-self-restore-refresh"
                  >
                    {managerSelfRestoreLoading ? '取得中…' : 'バックアップ時点を取得'}
                  </button>
                  <label className="basic-data-inline-field">
                    <span>戻したい時点</span>
                    <select value={selectedSelfRestoreBackup?.backupDateKey ?? ''} onChange={(event) => setSelfRestoreBackupDateKey(event.target.value)} disabled={managerSelfRestoreCandidates.length === 0} data-testid="backup-restore-self-restore-backup">
                      {managerSelfRestoreCandidates.length > 0 ? managerSelfRestoreCandidates.map((backup) => (
                        <option key={backup.backupDateKey} value={backup.backupDateKey}>{backup.displayLabel}</option>
                      )) : <option value="">まず「バックアップ時点を取得」を押してください</option>}
                    </select>
                  </label>
                  <button
