...
type StudentMenuPositionParams = {
  menu: Pick<StudentMenuState, 'x' | 'y' | 'mode'> | null
  viewportWidth: number
  viewportHeight: number
  popoverWidth: number
  popoverHeight: number
...
export type MakeupAutoAssignPendingItem = {
  subject: SubjectLabel
...
// 講習・振替の自動割振候補は matchedItem の型だけが異なる。規則評価の共有(下記
// findBestAutoAssignCandidate)で同一構造を扱うため、共通のジェネリックを正本にする。
type AutoAssignCandidate<TItem> = {
  weekIndex: number
  cell: SlotCell
  deskIndex: number
  studentIndex: number
...
type LectureAutoAssignCandidate = AutoAssignCandidate<LectureStockPendingItem>
type MakeupAutoAssignCandidate = AutoAssignCandidate<MakeupAutoAssignPendingItem>

type AutoAssignCandidateSearchResult<TItem> = {
  bestCandidate: AutoAssignCandidate<TItem> | null
  topCandidates: AutoAssignCandidate<TItem>[]
  evaluatedCandidateCount: number
}

type LectureAutoAssignCandidateSearchResult = AutoAssignCandidateSearchResult<LectureStockPendingItem>

// 振替ストック(科目別 raw)を「振るい順(古い origin から・balance 個)」で自動割振の pending items に展開する。
// 各 item は固定の振替元(makeupSourceDate/Label/ReasonLabel)を持ち、配置で1つ消費するたびに次の origin を使う。
// 回帰防止: balance を超えて生成しない(= 盤面再計算ベースの残数を超えて置かない保証)。
export function buildMakeupAutoAssignPendingItems(
  rawEntries: Array<Pick<MakeupStockEntry,
    'subject' | 'balance' | 'remainingOriginDates' | 'remainingOriginLabels' | 'remainingOriginReasonLabels' | 'nextOriginDate' | 'nextOriginLabel' | 'nextOriginReasonLabel'>>,
): MakeupAutoAssignPendingItem[] {
  const items: MakeupAutoAssignPendingItem[] = []
  for (const raw of rawEntries) {
    const count = Math.max(0, raw.balance)
    for (let index = 0; index < count; index += 1) {
      items.push({
        subject: raw.subject as SubjectLabel,
        makeupSourceDate: raw.remainingOriginDates[index] ?? raw.nextOriginDate ?? undefined,
        makeupSourceLabel: raw.remainingOriginLabels[index] ?? raw.nextOriginLabel ?? undefined,
        makeupSourceReasonLabel: raw.remainingOriginReasonLabels[index] ?? raw.nextOriginReasonLabel ?? undefined,
      })
    }
  }
  return items
...
const editableLessonTypes: LessonType[] = ['extra', 'regular', 'makeup', 'special']
...
function buildScheduleLocalStoragePrefix(viewType: 'student' | 'teacher', classroomStorageKey?: string) {
  const scope = encodeURIComponent(classroomStorageKey || 'default')
  return `schedule-shared:${scope}:${viewType}:range:`
}

function buildLegacyScheduleLocalStoragePrefix(viewType: 'student' | 'teacher') {
  return `schedule-shared:${viewType}:range:`
}

export function readStoredScheduleRange(viewType: 'student' | 'teacher', fallbackStartDate: string, fallbackEndDate: string, classroomStorageKey?: string): ScheduleRangePreference {
  try {
    const prefix = buildScheduleLocalStoragePrefix(viewType, classroomStorageKey)
    const legacyPrefix = buildLegacyScheduleLocalStoragePrefix(viewType)
    return {
...
function writeStoredScheduleRange(viewType: 'student' | 'teacher', range: ScheduleRangePreference, classroomStorageKey?: string) {
  try {
    const prefix = buildScheduleLocalStoragePrefix(viewType, classroomStorageKey)
...
function removeManualLectureStockOrigin(originMap: Record<string, ManualLectureStockOrigin[]>, key: string, options?: {
  sessionId?: string
...
function consumeManualLectureStockOrigin(originMap: Record<string, ManualLectureStockOrigin[]>, key: string, options?: {
  sessionId?: string
  originDateKey?: string
  originSlotNumber?: number
}) {
  ...
  const targetIndex = options?.originDateKey
    ? currentOrigins.findIndex((origin) => origin.originDateKey === options.originDateKey
      && origin.originSlotNumber === options.originSlotNumber
      && (!options.sessionId || origin.sessionId === options.sessionId))
    : options?.sessionId
  ...
  const nextManualLectureStockOrigins = item.source === 'manual'
    ? (item.sessionId
      ? removeManualLectureStockOrigin(manualLectureStockOrigins, item.stockKey, {
          ...
          originDateKey: item.originDateKey,
          originSlotNumber: item.originSlotNumber,
        })
      : removeManualLectureStockOrigin(manualLectureStockOrigins, item.stockKey))
...
// INV-06(Issue #57): **出欠記録(statusSlots)を破棄・上書きする操作**が、消える前に「振替コマの記録」を
...
export function reconcileHolidayDeskStockReturns(params: {
  ...
  for (let studentIndex = 0; studentIndex < statusSlots.length; studentIndex += 1) {
    ...
    if (statusEntry.lessonType === 'makeup' && statusEntry.makeupSourceDate) {
      // 確定処理は共通関数へ委譲(Issue #57 で移動・出欠付与の上書き経路と一本化。挙動不変)。
      const materializedResult = materializeDisplacedStatusEntryIntoLedgers({
        statusEntry,
...
export function appendDeletedStudentScheduleCountAdjustment(
...
//   2. テンプレ固定日(templateFreezeBeforeDate)より前のセルは再マージしない(INV-10 禁忌: テンプレ反映日以前のコマ表は不変)。
//   3. 固定日以降のセルだけ overlayBoardWeeksOnScheduleCells で再マージする。
export function remergeBoardWeekWithManagedData(rawWeek: SlotCell[], params: {
  classroomSettings: ClassroomSettings
  teachers: TeacherRow[]
  students: StudentRow[]
  regularLessons: RegularLessonRow[]
  ...
  const week = stripWithdrawnStudentsFromBoardWeek(rawWeek, params.students, params.todayKey)
...
function isReturnedToOriginalDate(student: StudentEntry, targetDateKey: string) {
  return Boolean(
    (student.makeupSourceDate && student.makeupSourceDate === targetDateKey)
    || (student.sameDayMoveSourceDate && student.sameDayMoveSourceDate === targetDateKey),
  )
}
...
export function prepareStudentForMove(student: StudentEntry, sourceDateKey: string, sourceSlotNumber: number, targetDateKey: string): StudentEntry {
  if (student.lessonType !== 'regular' && student.lessonType !== 'makeup') return { ...student }

  const originalDateKey = resolveOriginalRegularDate(student, sourceDateKey)
  const originalLabel = student.makeupSourceLabel ?? student.sameDayMoveSourceLabel ?? formatStockOriginLabel(originalDateKey, sourceSlotNumber)

  if (student.lessonType === 'regular' && originalDateKey === targetDateKey) {
    return {
      ...student,
      lessonType: 'regular',
      makeupSourceDate: undefined,
      makeupSourceLabel: undefined,
      sameDayMoveSourceDate: originalDateKey,
      sameDayMoveSourceLabel: originalLabel,
    }
  }

  return normalizeLessonPlacement({
    ...student,
    lessonType: 'makeup',
    makeupSourceDate: originalDateKey,
    makeupSourceLabel: originalLabel,
    sameDayMoveSourceDate: undefined,
    sameDayMoveSourceLabel: undefined,
  }, targetDateKey)
}
...
function buildStudentStatusEntry(student: StudentEntry, cell: SlotCell, desk: DeskCell, status: StudentStatusKind): StudentStatusEntry {
  return {
    ...
    sameDayMoveSourceDate: student.sameDayMoveSourceDate,
    sameDayMoveSourceLabel: student.sameDayMoveSourceLabel,
...
function buildStudentEntryFromStatus(statusEntry: StudentStatusEntry): StudentEntry {
  return {
    ...
    sameDayMoveSourceDate: statusEntry.sameDayMoveSourceDate,
    sameDayMoveSourceLabel: statusEntry.sameDayMoveSourceLabel,
...
export function buildBoardStudentSelectionOptions(students: StudentRow[], cellDateKey: string, managementReferenceDate = cellDateKey) {
  return students
    ...
    .map((student) => ({
      id: student.id,
      displayName: formatStudentSelectionLabel(student, cellDateKey),
      student,
    }))
}
...
export function buildTemplateStudentSelectionOptions(students: StudentRow[], templateReferenceDate: string) {
  return students
    ...
    .map((student) => ({
      id: student.id,
      displayName: formatStudentSelectionLabel(student, templateReferenceDate),
      student,
    }))
}
...
function buildCellDateSlotKey(cell: Pick<SlotCell, 'dateKey' | 'slotNumber'>) {
  return `${cell.dateKey}_${cell.slotNumber}`
}

function buildCellLookupMaps(cells: SlotCell[]) {
  return {
    byId: new Map(cells.map((cell) => [cell.id, cell])),
    byDateSlot: new Map(cells.map((cell) => [buildCellDateSlotKey(cell), cell])),
  }
}

function findMatchingBoardCell(cell: SlotCell, maps: ReturnType<typeof buildCellLookupMaps>) {
  return maps.byId.get(cell.id) ?? maps.byDateSlot.get(buildCellDateSlotKey(cell))
}
...
function overlayManualRegularAdditionsOnPlannedCells(scheduleCells: SlotCell[], boardWeeks: SlotCell[][]) {
  ...
  return scheduleCells.map((cell) => {
    const boardCell = findMatchingBoardCell(cell, boardCellMaps)
...
// 実体化済みの週(今日を含む)は「テンプレ沈黙セルの盤面授業を保持する」分岐(Issue #28)で残り続け、
// テンプレ固定日(templateFreezeBeforeDate)より前の週はそもそも再マージされない。そこで盤面の週に対して、
// **max(退塾日, 今日[JST]) 以降**の日付だけ、退塾生徒の**テンプレ由来の通常授業**を外す(固定日の前後を問わない)。
//   - ★昨日以前の盤面には触れない(過去の週を遡って削らない・オーナー決定 2026-09-15)。
...
//   - 机が空になったとき: 講師が manualTeacher、または机に出欠記録があれば講師を残す。そうでなければ
//     mergeManagedWeek の空き机と同じく講師を外す(INV-01: 今日以降に限りオーナー許容・盤面=講師日程表)。
//   - これはテンプレの再マージではない(固定日前の週でも剥がしだけを行い、テンプレから講師・授業を足さない=INV-10)。
...
export function stripWithdrawnStudentsFromTemplateRegularLessons(
  ...
  if (cell.dateKey < todayKey) return cell
  ...
  nextCell.desks = nextCell.desks.map((desk) => {
    ...
    if (nextStudentSlots[0] || nextStudentSlots[1]) return { ...desk, lesson: { ...desk.lesson, studentSlots: nextStudentSlots } }
    if (desk.manualTeacher || hasRecordedStatusSlots(desk)) return { ...desk, lesson: undefined }
    return {
      ...desk,
      teacher: '',
      manualTeacher: false,
...
// 盤面の 1 週(または任意のセル列)に剥がしを適用する。変化が無ければ同じ配列を返す。
export function stripWithdrawnStudentsFromBoardWeek(
  week: SlotCell[],
  students: ReadonlyArray<Pick<StudentRow, 'id' | 'withdrawDate'>>,
  todayKey: string,
): SlotCell[] {
  if (students.length === 0) return week
  let changed = false
  const next = week.map((cell) => {
    const stripped = stripWithdrawnStudentsFromTemplateRegularLessons(cell, students, todayKey)
    if (stripped !== cell) changed = true
    return stripped
  })
  return changed ? next : week
}

// ── 退塾スイープ(オーナー確定 2026-09-20・確認リスト b-2) ──────────────────────────────────────
// 基本データの「退塾」ボタンを押した生徒の **fromDateKey(= max(退塾日, 今日[JST])) 以降**の盤面の痕跡を消し切る純関数。
// 消すもの: その生徒の席(studentSlots ＝ 手で置いた講習・振替・増コマ・体験・手動追加・移動、通常授業も)と出欠記録(statusSlots)。
//   ・通常授業(テンプレ由来)は stripWithdrawnStudentsFromBoardWeek も剥がす。ここで消すのは二重の安全網で、
//     手動追加・同日移動など**剥がしの対象外**の通常授業もここで消える(オーナー確定 2026-09-20)。
// ★昨日以前のセルには一切触らない(出欠記録は請求・通常授業履歴の根拠)。変更が無かったセル・週は**同じ参照**で返す
//   (呼び出し側は changed=false なら commitWeeks しない ＝ 空の Undo を積まない)。
// ★他の生徒の席・講師(manualTeacher)・メモは不変。席が空いた机の講師は既存の削除系(handleDeleteStudent)と同じく
//   触らない(INV-01: 講師の帰属をこの掃除で動かさない)。
// ★在庫(INV-06): **未消化へは戻さない**。台帳(manualMakeupAdjustments / manualLectureStockCounts)へは一切足さず、
//   「破棄前に台帳へ確定」(materializeDisplacedStatusEntryIntoLedgers)もしない。代わりに、**席・記録を消したことで
//   在庫が湧く分だけ**抑止(suppressedMakeupOrigins)へ積む:
//     - 振替コマ(makeup・makeupSourceDate あり) … 盤面から外れると消化(collectMakeupUsageByKey)が減って台帳 origin が
//       再浮上し未消化 +1 になるので、その振替元日を抑止する(1コマ削除 handleDeleteStudent と同じ流儀＝日付ワイルドカード)。
//       manualAdded の振替と、消化に数えない記録(absent / holiday)は再浮上しないので積まない。
//     - 通常授業 … 後からその日を休日設定したときの自動計上(computeAutomaticShortageOrigins)が湧かないよう元の通常授業日を
//       抑止する(Issue #58 の全コマ削除と同じ)。★absent / moved / holiday の記録には積まない
//       (立っている在庫や移動先が持つ会計を消す誤減になる ＝ collectClearedDayMakeupSuppressions と同じ guard)。
//   ★抑止は**日付だけ**(時限なしのワイルドカード)で積む: 1コマ削除 handleDeleteStudent と同じ流儀。時限つきで積むと、
//     台帳側が時限なしトークンの origin(旧データ・日付だけの手動調整)に当たらず未消化が湧く(誤増)。同じ日の別コマの
//     origin まで巻き込む可能性はあるが、キーは「その生徒×科目」なので他の生徒には及ばず、退塾で消し切る意図にも沿う。
...
export function computeStudentWithdrawSweep(params: {
  weeks: SlotCell[][]
  /** 名簿(managedStudentId を持たない古い席を「名簿で一意な名前」で拾うため)。 */
  ...
  if (!studentId || !fromDateKey) return unchanged

  // 生徒の同一性は保護者連絡の自動処理と同じ決め方(managedStudentId 最優先・無ければ名簿で一意な名前)。
  const ownerByName = buildUniqueNameOwnerMap(students)
  const owns = (entry: Pick<StudentEntry, 'managedStudentId' | 'name'> | null | undefined) => (
    Boolean(entry && isBoardStudentOwnedBy(entry, studentId, ownerByName))
  )
  const isSweepTargetCell = (cell: SlotCell) => Boolean(cell.dateKey) && cell.dateKey >= fromDateKey
  const cellHasTrace = (cell: SlotCell) => cell.desks.some((desk) => (
    (desk.lesson?.studentSlots ?? []).some((student) => owns(student))
    || (desk.statusSlots ?? []).some((entry) => owns(entry))
  ))

  let removedSeatCount = 0
  let removedStatusCount = 0
  let nextSuppressedMakeupOrigins = params.suppressedMakeupOrigins
  const appendedSuppressionTokens = new Set<string>()
  // 席・記録 1 件分の抑止(上のコメントの表がそのまま条件)。status は出欠記録を消すときだけ渡す。
  const suppressForRemovedEntry = (entry: StudentEntry, cellDateKey: string, status?: StudentStatusKind) => {
    const originDate = entry.lessonType === 'makeup'
      ? (entry.manualAdded || status === 'absent' || status === 'holiday' ? '' : entry.makeupSourceDate ?? '')
      : entry.lessonType === 'regular'
        ? (status === 'absent' || status === 'moved' || status === 'holiday' ? '' : resolveOriginalRegularDate(entry, cellDateKey))
        : ''
    if (!originDate) return
    const stockKey = buildMakeupStockKey(resolveStockId(entry), entry.subject)
    const token = `${stockKey}|${originDate}`
    if (appendedSuppressionTokens.has(token)) return
    appendedSuppressionTokens.add(token)
    nextSuppressedMakeupOrigins = appendMakeupOrigin(nextSuppressedMakeupOrigins, stockKey, originDate)
  }

  let changed = false
  const nextWeeks = weeks.map((week) => {
    let weekChanged = false
    const nextWeek = week.map((cell) => {
      if (!isSweepTargetCell(cell) || !cellHasTrace(cell)) return cell
      weekChanged = true
      changed = true
      const nextCell = cloneSlotCell(cell)
      for (const desk of nextCell.desks) {
        // 席: index を先に集めてから削る(removeStudentFromDeskLesson が最後の1人で lesson を落とすため)。
        const ownedSeatIndexes = (desk.lesson?.studentSlots ?? [])
          .map((student, studentIndex) => (owns(student) ? studentIndex : -1))
          .filter((studentIndex) => studentIndex >= 0)
        for (const studentIndex of ownedSeatIndexes) {
          const seat = desk.lesson?.studentSlots[studentIndex]
          if (!seat) continue
          suppressForRemovedEntry(seat, nextCell.dateKey)
          removeStudentFromDeskLesson(desk, studentIndex)
          removedSeatCount += 1
        }
        // 出欠記録: 同じ机の他の生徒の記録は残す(setDeskStudentStatus が全部 null なら statusSlots を落とす)。
        const ownedStatusIndexes = (desk.statusSlots ?? [])
          .map((entry, statusIndex) => (owns(entry) ? statusIndex : -1))
          .filter((statusIndex) => statusIndex >= 0)
        for (const statusIndex of ownedStatusIndexes) {
          const statusEntry = desk.statusSlots?.[statusIndex]
          if (!statusEntry) continue
          suppressForRemovedEntry(buildStudentEntryFromStatus(statusEntry), nextCell.dateKey, statusEntry.status)
...
export function overlayBoardWeeksOnScheduleCells(scheduleCells: SlotCell[], boardWeeks: SlotCell[][], explicitlySuppressedManagedKeys: string[] = []) {
  ...
  for (const boardCell of boardWeeks.flat()) {
    ...
    if (managedCellDateSlots.has(buildCellDateSlotKey(boardCell))) continue
...
export function clampPopoverPosition({
  ...
  return {
    left: Math.max(minLeft, Math.min(anchorX + offsetX, viewportWidth - popoverWidth - rightMargin)),
    top: Math.max(minTop, Math.min(anchorY + offsetY, viewportHeight - popoverHeight - bottomMargin)),
  }
}
...
export function resolveStudentMenuPosition({
  ...
  if (menu.mode === 'edit') {
    return {
      left: Math.max(12, Math.min(menu.x + 10, viewportWidth - popoverWidth - 16)),
      top: 16,
    }
  }
  ...
  return clampPopoverPosition({
    ...
    popoverWidth,
    popoverHeight,
    bottomMargin: menu.mode === 'empty' ? 80 : 12,
  })
}
...
export function buildScheduleCellsForRange(params: {
  ...
  boardOnly?: boolean
  // 退塾生徒の剥がし(stripWithdrawnStudentsFromBoardWeek)の「今日」。省略時は JST の今日。テスト用に注入できる。
  todayKey?: string
}) {
  ...
  const boardWeeks = params.boardWeeks.map((week) => stripWithdrawnStudentsFromBoardWeek(week, params.students, todayKey))

  return overlayBoardWeeksOnScheduleCells(managedCells, boardWeeks, params.suppressedRegularLessonOccurrences ?? [])
}
...
function mergeManagedWeek(currentWeek: SlotCell[], managedWeek: SlotCell[], originalManagedWeek?: SlotCell[]) {
  ...
  return currentWeek.map((cell) => {
    ...
    const originalManagedDesksByLessonId = new Map(
      originalManagedCell.desks
        .filter((desk) => desk.lesson && isManagedLesson(desk.lesson))
        .map((desk) => [desk.lesson!.id, desk]),
    )
      ...
      if (hasRegularStudentToPreserve) {
        ...
        if (!originalSameLessonWasSuppressed && (!sameDeskManagedLessonIsManaged || (sameDeskManagedLessonConsumedElsewhere && hasRegularStudentMissingFromManagedCell))) {
          ...
          return {
            ...desk,
            statusSlots: cloneStatusSlots(desk.statusSlots),
            lesson: cloneDeskLesson(lesson),
          }
        }
      }

      // 管理セルにmanaged lessonが一切無い（テンプレがこのコマについて沈黙している）場合、
      // 盤面のレッスンをそのまま保持して日程表と盤面表示を一致させる。
      // 旧テンプレ由来のmanaged_*レッスンがコマ表に残っており、新テンプレに対応行が無いケースで
      // 日程表側の通常生徒が脱落していた不具合への対処。
      // ※抑止により managedDesksByLessonId が空になっただけのケースは除外する。
      if (isTemplateSilentForCell) {
        preservedLessonIds.add(lesson.id)
        return {
          ...desk,
          statusSlots: cloneStatusSlots(desk.statusSlots),
        ...
        if (lesson) {
          ...
          if (params.restoreSessionStock && studentEntry.specialStockSource === 'session') {
            ...
            nextManualLectureStockOrigins = appendManualLectureStockOrigin(nextManualLectureStockOrigins, lectureStockKey, {
              ...
              sessionId: params.session.id,
              originDateKey: studentEntry.makeupSourceDate ?? cell.dateKey,
              originSlotNumber: parseOriginSlotNumber(studentEntry.makeupSourceLabel) ?? cell.slotNumber,
            })
...
export type ComputeStudentMoveResult =
  ...
  // displacedStatusEntries = この移動が上書きで消した出欠記録(moved マーカー書き込み時の前記録・Issue #57)。
  // 呼び出し側は materializeDisplacedStatusEntryIntoLedgers で台帳確定してから commit すること(INV-06)。
  | { status: 'moved'; message: string; nextWeeks: SlotCell[][]; nextSuppressedRegularLessonOccurrences: string[]; displacedStatusEntries: StudentStatusEntry[] }
...
export function computeStudentMove(params: {
  ...
  // Issue #57: この移動が上書きで消した出欠記録(呼び出し側で台帳確定させる)。
  const displacedStatusEntries: StudentStatusEntry[] = []
  ...
  // 回帰防止(ee5728c): 移動先スロットに残った「移)日付」マーカーは新しい生徒が引き継いで表示されるため消す。
  ...
  if (!swapStudent && sourceDateKey !== targetCell.dateKey && movedStatusStudent.lessonType === 'regular') {
    ...
    if (sourceCell && sourceDesk) {
      // Issue #57: moved マーカーで上書きすると、このスロットに保持されていた前の生徒の出欠記録が消える。
  ...
  return { status: 'moved', message, nextWeeks, nextSuppressedRegularLessonOccurrences, displacedStatusEntries }
}
...
export function ScheduleBoardScreen({ classroomSettings, classroomName, classroomStorageKey, teachers, students, regularLessons, specialSessions, autoAssignRules, pairConstraints, teacherAutoAssignRequest, onTeacherAutoAssignRequestProcessed, studentScheduleRequest, onStudentScheduleRequestProcessed, parentAbsenceRequest, onParentAbsenceRequestProcessed, onParentAbsencePlacementSettled, studentWithdrawSweepRequests, onStudentWithdrawSweepProcessed, initialBoardState, onBoardStateChange, onReplaceRegularLessons, onUpdateSpecialSessions, onApplyReopenedSlots, onUpdateClassroomSettings, onOpenBasicData, onOpenSpecialData, onOpenAutoAssignRules, onOpenBackupRestore, onPreTemplateSaveBackup, undoSnapshotLabel, onRestoreUndoSnapshot, onDismissUndoSnapshot, onLogout, onCopyDistributionUrl, onReportToDeveloper, onOpenParentContactHistory, parentContactUnconfirmedCount, onSaveBoard, isBoardDirty, isBoardSaving, isBoardSaveDisabled, hasPendingSave, syncStatusMessage, syncProgressPercent, syncElapsedSeconds, onDeletionStockSummaryChange }: ScheduleBoardScreenProps) {
  ...
  const studentScheduleOptionFieldEnabled = isFeatureEnabledForClassroom('studentScheduleOptionField', { id: classroomStorageKey })
  // 【移行中・INV-05】通常の予定数を盤面ベース（実績＋未振替の休み）で出すか。開発用教室から段階導入。
  const boardBasedPlannedCountEnabled = isFeatureEnabledForClassroom('boardBasedPlannedCount', { id: classroomStorageKey })
  // 【INV-01/INV-02】日程表を盤面そのままで描く(テンプレ再マージを行わない)。全教室で有効(2026-08-07)。
  const scheduleBoardOnlyEnabled = isFeatureEnabledForClassroom('boardOnlyScheduleCells', { id: classroomStorageKey })
  // 生徒名の長押しD&D移動は開発用教室のみ先行有効(検証後に全教室へ昇格予定)。
  const studentDragMoveEnabled = isFeatureEnabledForClassroom('studentDragAndDropMove', { id: classroomStorageKey })
  // 講師の長押しD&D移動/入れ替え(同一コマ内限定)は開発用教室のみ先行有効(検証後に全教室へ昇格予定)。
  const teacherDragMoveEnabled = isFeatureEnabledForClassroom('teacherDragAndDropMove', { id: classroomStorageKey })
  ...
  const scheduleDndMoveEnabled = isFeatureEnabledForClassroom('studentScheduleDndMove', { id: classroomStorageKey })
  // 別タブ日程表の自動同期(デバウンス)＋同期スピナーも staging/開発用教室のみ。本番3教室は従来どおり
  // 「最新表示」ボタン/開いた時のみ更新に保つ(オーナー確定 2026-07-09: メモリ負荷が本番大教室で未検証のため)。
  const scheduleAutoSyncEnabled = isFeatureEnabledForClassroom('schedulePopupAutoSync', { id: classroomStorageKey })
  // 盤面PDFのコマ選択(A3縦は据え置き)。OFF の教室は従来どおりモーダルなしで表示週まるごと即出力する。
  const boardPrintSelectionEnabled = isFeatureEnabledForClassroom('boardPrintSelection', { id: classroomStorageKey })
  ...
  // 講習の自動割振モーダルで「未消化振替も同時に自動割り当てする」かどうか。既定 OFF(オーナー確定 2026-06-26)。
  const [includeMakeupInAutoAssign, setIncludeMakeupInAutoAssign] = useState(false)
  ...
  const handleSaveRegularLessonTemplate = useCallback((template: RegularLessonTemplate, overwrite: boolean) => {
    ...
    if (overwrite) {
      ...
      onBoardStateChange?.({
        ...
        manualMakeupAdjustments: cloneOriginMap(nextManualMakeupAdjustments),
        suppressedMakeupOrigins: cloneOriginMap(nextSuppressedMakeupOrigins),
  ...
  useEffect(() => {
    ...
    // 初回評価(basis=null)・specialSessions 未ロード(空)は除去せず基準記録に留める。空 Set 初期化だと
    // 既存の未提出配置=保存済みの自動割振済み講習まで初回に消える回帰(2026-07-06)の原因になるため、
    // 基準取り込みの判断を純関数 resolveNewlyUnsubmittedSessionStudents に一本化(nextBasisKeys を据え置く)。
  ...
  useEffect(() => {
    const handleScheduleNoteMessage = (event: MessageEvent) => {
      ...
      const noteId = typeof message.noteId === 'string' && message.noteId.trim().length > 0
        ? message.noteId
        : typeof message.noteKey === 'string' && message.noteKey.trim().length > 0
          ? `${message.viewType}:${message.noteKey}`
          : ''
      if (!noteId) return
  ...
  const teacherUnavailableSlotsById = useMemo(() => {
    const byId = new Map<string, Set<string>>()
    for (const session of specialSessions) {
      for (const [teacherId, input] of Object.entries(session.teacherInputs)) {
  ...
  // 講習・振替で共有する候補探索の核(回帰防止の肝)。セル×机×生徒スロットの規則評価は完全共通で、
  // 差分は3点だけをコールバックで注入する:
  //   1. selectMatchedItem … そのセルで割り当てる pending item の絞り込み(期間・出席不可など)
  //   2. buildLeadingScoreParts … スコア先頭の種別固有パート(講習: 未消化講習由来 / 振替: なし)
  //   3. buildTrailingScoreParts … スコア末尾の種別固有パート(講習: 講習終了日優先 / 振替: 振るい順優先)
  // 中段(絶対事項・共通スコア・日付優先)は常に同一順序で構築する。講習側の出力は従来と byte 一致。
  const findBestAutoAssignCandidate = <TItem extends { subject: SubjectLabel }>(params: {
    sourceWeeks: SlotCell[][]
    managedStudent: StudentRow
    studentKey: string
    selectMatchedItem: (context: {
      cell: SlotCell
      teacher: TeacherRow
      studentGradeOnDate: GradeLabel
      slotKey: string
      studentUnavailableSlots: Set<string>
    }) => TItem | null
    buildLeadingScoreParts: (matchedItem: TItem) => AutoAssignScorePart[]
    buildTrailingScoreParts: (matchedItem: TItem) => AutoAssignScorePart[]
  }): AutoAssignCandidateSearchResult<TItem> => {
    const studentUnavailableSlots = studentUnavailableSlotsById.get(params.managedStudent.id) ?? new Set<string>()
    let bestCandidate: AutoAssignCandidate<TItem> | null = null
    const evaluatedCandidates: AutoAssignCandidate<TItem>[] = []
      ...
      for (const cell of week) {
        ...
        for (let deskIndex = 0; deskIndex < cell.desks.length; deskIndex += 1) {
          ...
          for (let studentIndex = 0; studentIndex < 2; studentIndex += 1) {
            ...
            const matchedItem = params.selectMatchedItem({ cell, teacher, studentGradeOnDate, slotKey, studentUnavailableSlots })
            ...
            const scoreParts: AutoAssignScorePart[] = [
              ...
              ...params.buildTrailingScoreParts(matchedItem),
            ]
            const scoreVector = scoreParts.map((part) => part.value)

            const nextCandidate: AutoAssignCandidate<TItem> = {
  ...
  const findBestLectureAutoAssignCandidate = (params: {
    ...
    managedStudent: StudentRow
    studentKey: string
  }): LectureAutoAssignCandidateSearchResult => findBestAutoAssignCandidate<LectureStockPendingItem>({
    sourceWeeks: params.sourceWeeks,
    managedStudent: params.managedStudent,
    studentKey: params.studentKey,
    selectMatchedItem: ({ cell, teacher, studentGradeOnDate, slotKey, studentUnavailableSlots }) => [...params.pendingItems]
      .filter((item) => {
    ...
    buildLeadingScoreParts: (matchedItem) => [
      {
        label: '未消化講習由来',
    ...
    buildTrailingScoreParts: (matchedItem) => [
      {
        label: '講習終了日優先',
  ...
  // 未消化振替の自動割振。講習(findBestLectureAutoAssignCandidate)と同一の規則・スコア中段を共有し、差分は
  //   - 対象セルは「トリガーした講習の期間内」のみ(session.startDate〜endDate)
  //   - 振るい順(古い origin)を優先する末尾スコア(講習終了日優先と同じ作り)
  // の2点だけ。matchedItem(=振替元固定の pending item)は科目対応→振るい順→科目名で選ぶ。
  const findBestMakeupAutoAssignCandidate = (params: {
    ...
    studentKey: string
    session: SpecialSessionRow
  }): AutoAssignCandidateSearchResult<MakeupAutoAssignPendingItem> => findBestAutoAssignCandidate<MakeupAutoAssignPendingItem>({
    sourceWeeks: params.sourceWeeks,
    managedStudent: params.managedStudent,
    studentKey: params.studentKey,
    selectMatchedItem: ({ cell, teacher, studentGradeOnDate, slotKey, studentUnavailableSlots }) => {
      if (!isDateWithinRange(cell.dateKey, params.session.startDate, params.session.endDate)) return null
      if (studentUnavailableSlots.has(slotKey)) return null
      return [...params.pendingItems]
        .sort((left, right) => {
          const leftCapable = canTeacherHandleStudentSubject(teacher, left.subject, studentGradeOnDate)
          const rightCapable = canTeacherHandleStudentSubject(teacher, right.subject, studentGradeOnDate)
          if (leftCapable !== rightCapable) return leftCapable ? -1 : 1
    ...
    buildLeadingScoreParts: () => [],
    buildTrailingScoreParts: (matchedItem) => [
      {
        label: '振替元の振るい順優先',
        value: matchedItem.makeupSourceDate ? 99999999 - Number(matchedItem.makeupSourceDate.replace(/-/g, '')) : 0,
        detail: matchedItem.makeupSourceDate ? `${matchedItem.makeupSourceDate} の振替を優先（古い順）` : '振替元なし',
      },
    ],
  })
  ...
  const buildBoardWeeksForScheduleRange = useCallback((range: ScheduleRangePreference) => {
    const normalizedRange = normalizeScheduleRange(range, scheduleFallbackStartDate, scheduleFallbackEndDate)
  ...
  const displayCells = useMemo(() => {
    ...
    const slotKeyForCell = (dateKey: string, slotNumber: number) => `${dateKey}_${slotNumber}`
    const resolveTeacherIdForDesk = (desk: DeskCell) => {
      if (desk.teacherAssignmentTeacherId) return desk.teacherAssignmentTeacherId
      if (!desk.teacher) return undefined
      const matched = teachers.find((t) => getTeacherDisplayName(t) === desk.teacher || t.name === desk.teacher)
      return matched?.id
    }
    return sourceCells.map((cell) => ({
      ...
      desks: cell.desks.map((desk, deskIndex) => {
        ...
        const resolvedTeacher = teacherWithdrawn ? '' : desk.teacher
        let teacherUnavailableWarning = false
        if (!isTemplateMode && resolvedTeacher) {
          const teacherId = resolveTeacherIdForDesk(desk)
          if (teacherId) {
            const unavailableSlots = teacherUnavailableSlotsById.get(teacherId)
            if (unavailableSlots?.has(slotKeyForCell(cell.dateKey, cell.slotNumber))) {
  ...
  }, [boardStudentWarningsByLocation, cells, isTeacherWithdrawnOnDate, isTemplateMode, templateCells, teacherUnavailableSlotsById, teachers])
  ...
  useLayoutEffect(() => {
    ...
    const popoverElement = studentMenuPopoverRef.current
    ...
    const updateSize = () => {
      ...
      setStudentMenuPopoverSize((current) => {
  ...
  const menuPosition = useMemo(() => {
    ...
    const fallbackWidth = studentMenu.mode === 'memo' ? 280 : 320
    const popoverWidth = studentMenuPopoverSize?.width ?? fallbackWidth
    const popoverHeight = studentMenuPopoverSize?.height ?? estimatedHeight

    return resolveStudentMenuPosition({
      menu: studentMenu,
  ...
  }, [studentMenu, studentMenuPopoverSize])
  ...
  const templateAddableStudents = useMemo(() => {
    ...
    return buildTemplateStudentSelectionOptions(students, templateEffectiveStartDate)
  ...
  const handleSelectDesk = (cellId: string, deskIndex: number, x: number, y: number) => {
    ...
    const resolvedName = options.some((option) => option.name === initialName)
      ? initialName
      : (options[0]?.name ?? '')
  ...
  const handlePlaceLectureFromStock = (cellId: string, deskIndex: number, studentIndex: number) => {
    ...
    const nextStudent: StudentEntry = {
      ...
      grade: studentGrade,
      birthDate: managedStudent?.birthDate,
      makeupSourceDate: placementEntry.originDateKey,
      makeupSourceLabel: placementEntry.originDateKey && placementEntry.originSlotNumber
        ? formatStockOriginLabel(placementEntry.originDateKey, placementEntry.originSlotNumber)
        : undefined,
    ...
    const nextManualLectureStockOrigins = placementEntry.sessionId
      ? consumeManualLectureStockOrigin(manualLectureStockOrigins, lectureStockKey, {
          sessionId: placementEntry.sessionId,
          originDateKey: placementEntry.originDateKey,
          originSlotNumber: placementEntry.originSlotNumber,
        })
      : consumeManualLectureStockOrigin(manualLectureStockOrigins, lectureStockKey)
    commitWeeks(
      nextWeeks,
  ...
  const handleAutoAssignLectureStockEntry = (entry: GroupedLectureStockEntry, options: { includeMakeup?: boolean } = {}) => {
    ...
    while (remainingItems.length > 0) {
      ...
      const nextStudent: StudentEntry = {
        ...
        grade: studentGrade,
        birthDate: managedStudent.birthDate,
        makeupSourceDate: candidate.matchedItem.originDateKey,
        makeupSourceLabel: candidate.matchedItem.originDateKey && candidate.matchedItem.originSlotNumber
          ? formatStockOriginLabel(candidate.matchedItem.originDateKey, candidate.matchedItem.originSlotNumber)
          : undefined,
      ...
      nextManualLectureStockOrigins = candidate.matchedItem.sessionId
        ? consumeManualLectureStockOrigin(nextManualLectureStockOrigins, lectureStockKey, {
            sessionId: candidate.matchedItem.sessionId,
            originDateKey: candidate.matchedItem.originDateKey,
            originSlotNumber: candidate.matchedItem.originSlotNumber,
          })
        : consumeManualLectureStockOrigin(nextManualLectureStockOrigins, lectureStockKey)
    ...
    // 選択式: 講習を全て配置し終えた後で、同じ生徒の未消化振替を「振るい順」から同一 nextWeeks 上に割り振る。
    // 対象セルはトリガーした講習(session)の期間内のみ。割り振り切れない分はストックに残す(エラーにしない)。
    const makeupPlacedItems: Array<{ dateLabel: string; slotLabel: string; deskLabel: string }> = []
    const makeupPlacementCandidates: MakeupAutoAssignCandidate[] = []
    let remainingMakeupCount = 0
    if (options.includeMakeup && entry.studentId) {
      const makeupSession = entry.sessionId ? specialSessions.find((session) => session.id === entry.sessionId) ?? null : null
      const makeupGroup = makeupStockEntries.find((group) => group.studentId === entry.studentId) ?? null
      if (makeupSession && makeupGroup) {
        const rawMakeupEntries = rawMakeupStockEntries.filter(
          (raw) => getStockStudentKeyFromEntryKey(raw.key) === makeupGroup.stockStudentKey && raw.balance > 0,
        )
        const remainingMakeupItems = buildMakeupAutoAssignPendingItems(rawMakeupEntries)
        while (remainingMakeupItems.length > 0) {
          const makeupSearch = findBestMakeupAutoAssignCandidate({
            sourceWeeks: nextWeeks,
            pendingItems: remainingMakeupItems,
            managedStudent,
            studentKey,
            session: makeupSession,
          })
          const makeupCandidate = makeupSearch.bestCandidate
          if (!makeupCandidate) break

          const targetCell = nextWeeks[makeupCandidate.weekIndex]?.find((cell) => cell.id === makeupCandidate.cell.id)
          const targetDesk = targetCell?.desks[makeupCandidate.deskIndex]
          if (!targetCell || !targetDesk) break

          const studentGrade = resolveSchoolGradeLabel(managedStudent.birthDate, parseDateKey(targetCell.dateKey))
          // 手動配置(handlePlaceMakeupFromStock)と同じ normalizeLessonPlacement / origin・note 付与を踏襲する(回帰防止)。
          const nextStudent: StudentEntry = normalizeLessonPlacement({
            id: createStudentId(targetCell.id, makeupCandidate.deskIndex, makeupCandidate.studentIndex),
            name: getStudentDisplayName(managedStudent),
            managedStudentId: managedStudent.id,
            grade: studentGrade,
            birthDate: managedStudent.birthDate,
            makeupSourceDate: makeupCandidate.matchedItem.makeupSourceDate,
            makeupSourceLabel: makeupCandidate.matchedItem.makeupSourceLabel,
            subject: makeupCandidate.matchedItem.subject,
            lessonType: 'makeup',
            teacherType: 'normal',
          }, targetCell.dateKey)
          ...
          if (!targetDesk.lesson) {
            targetDesk.lesson = {
              id: `${targetCell.id}_desk_${makeupCandidate.deskIndex + 1}_${nextStudent.lessonType}`,
              note: nextStudent.lessonType === 'makeup' && makeupCandidate.matchedItem.makeupSourceLabel
                ? `元の通常授業: ${makeupCandidate.matchedItem.makeupSourceLabel}${makeupCandidate.matchedItem.makeupSourceReasonLabel ? `（${makeupCandidate.matchedItem.makeupSourceReasonLabel}）` : ''}`
                : undefined,
              studentSlots: [null, null],
            }
          }

          targetDesk.lesson.studentSlots[makeupCandidate.studentIndex] = nextStudent
          const matchedIndex = remainingMakeupItems.indexOf(makeupCandidate.matchedItem)
          if (matchedIndex >= 0) remainingMakeupItems.splice(matchedIndex, 1)
          makeupPlacementCandidates.push(makeupCandidate)
          makeupPlacedItems.push({
            dateLabel: targetCell.dateLabel,
            slotLabel: targetCell.slotLabel,
            deskLabel: resolveDeskLabel(targetDesk, makeupCandidate.deskIndex),
          })
        }
        remainingMakeupCount = remainingMakeupItems.length
      }
    }

    if (placedItems.length === 0 && makeupPlacedItems.length === 0) {
    ...
    setStatusMessage(
      options.includeMakeup
        ? (
          `${entry.displayName} を自動割振しました。講習${placedItems.length}コマ・振替${makeupPlacedItems.length}コマ配置しました。`
          + (remainingItems.length > 0 ? ` 講習${remainingItems.length}コマは候補不足でストックに残しています。` : '')
          + (remainingMakeupCount > 0 ? ` 振替${remainingMakeupCount}コマは候補不足でストックに残しています。` : '')
        )
        : (
  ...
  const executeMoveStudent = (cellId: string, deskIndex: number, studentIndex: number, sourceStudentIdOverride?: string | null) => {
    ...
    let nextFallbackMakeupStudents = fallbackMakeupStudents
    for (const displaced of result.displacedStatusEntries) {
      const materializedResult = materializeDisplacedStatusEntryIntoLedgers({
        statusEntry: displaced,
  ...
  // ==== 日程表コマ組み(spec-student-schedule-dnd) ====
  // 生徒日程表(別タブ・生成HTML)のD&D→机選択の確定で呼ばれる。盤面の computeStudentMove を直接使い、
  ...
  const handleStudentClick = (cellId: string, deskIndex: number, studentIndex: number, hasStudent: boolean, hasMemo: boolean, _statusKind: StudentStatusKind | null, x: number, y: number) => {
    ...
    if (!selectedStudentId) {
      ...
      setMemoDraft(currentMemo)
      setStudentMenu({ cellId, deskIndex, studentIndex, x, y, mode: 'empty' })
      setStatusMessage(currentStatus ? `${getStudentStatusActionLabel(currentStatus.status)}セルのメニューを開きました。` : '空欄メニューを開きました。')
      return
    }
  ...
  const handleOpenStudentEdit = () => {
    ...
    setEditStudentDraft({
      ...
      teacherType: menuStudent.student.teacherType,
      noteSuffix: normalizeRegularLessonNote(menuStudent.student.noteSuffix),
    })
    setStudentMenu({ ...studentMenu, mode: 'edit' })
  }
  ...
  const handleCopyDistributionUrl = async () => {
    if (!onCopyDistributionUrl) {
      setStatusMessage('配布用URLを作成できません。')
      return
    }

    setDistributionQrModal({ url: '', svg: '', isLoading: true })
    try {
      const url = await onCopyDistributionUrl()
      setDistributionQrModal({ url, svg: generateQrSvg(url, 220), isLoading: false })
      setStatusMessage(`配布用URLをコピーしました: ${url}`)
    } catch (error) {
      setDistributionQrModal(null)
      setStatusMessage(`配布用URLの作成に失敗しました: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  const handleStoreStudent = () => {
    ...
    if (menuStudent.student.lessonType === 'special') {
      ...
      const nextManualLectureStockOrigins = appendManualLectureStockOrigin(manualLectureStockOrigins, lectureStockKey, {
        ...
        originDateKey: menuStudent.student.makeupSourceDate ?? targetCell.dateKey,
        originSlotNumber: parseOriginSlotNumber(menuStudent.student.makeupSourceLabel) ?? targetCell.slotNumber,
      })
  ...
  const markStudentAbsentAt = (seat: ParentAbsenceTarget): { makeupStock: { stockKey: string; originDate: string; originSlotNumber: number | null } | null } | null => {
    ...
    if (targetStudent.lessonType === 'special') {
      if (!shouldReturnLectureStockOnAbsence(targetStudent)) {
        commitWeeks(
          ...
          suppressedMakeupOrigins,
          displacedLedgers.fallbackMakeupStudents,
          manualLectureStockCounts,
          manualLectureStockOrigins,
          fallbackLectureStockStudents,
        )
      ...
      const nextManualLectureStockOrigins = appendManualLectureStockOrigin(manualLectureStockOrigins, lectureStockKey, {
        ...
        originDateKey: targetCell.dateKey,
        originSlotNumber: targetCell.slotNumber,
      })
    ...
    const nextManualMakeupAdjustments = (targetStudent.lessonType === 'regular' || !targetStudent.makeupSourceDate)
      ? appendMakeupOrigin(displacedLedgers.manualMakeupAdjustments, stockKey, resolveOriginalRegularDate(targetStudent, targetCell.dateKey))
      : displacedLedgers.manualMakeupAdjustments
  ...
  const handleClearStudentStatus = () => {
    ...
    const restoredStudent = clearStudentStatusFromDesk(targetDesk, studentMenu.studentIndex, statusEntry)
  ...
  const jumpToWeekByDate = (dateKey: string): boolean => {
    ...
    const coveredWeeks = ensureWeeksCoverDateRange({
      weeks,
      startDate: targetWeekStartKey,
      endDate: targetWeekEndKey,
      classroomSettings,
      teachers,
      students,
      regularLessons,
        ...
        <div className="distribution-qr-modal-overlay" onClick={(event) => { if (event.target === event.currentTarget) setDistributionQrModal(null) }}>
          <div className="distribution-qr-modal" role="dialog" aria-modal="true" aria-label="講師日程共有 QR" data-testid="distribution-qr-modal">
            ...
            {distributionQrModal.isLoading ? (
              ...
              <>
                <div className="distribution-qr-code" dangerouslySetInnerHTML={{ __html: distributionQrModal.svg }} />
                <div className="distribution-qr-url">{distributionQrModal.url}</div>
              </>
            )}
            <div className="distribution-qr-actions">
              <button className="primary-button slim" type="button" onClick={() => setDistributionQrModal(null)} data-testid="distribution-qr-close-button">閉じる</button>
            </div>
          </div>
        </div>
      ) : null}
        ...
        <section className="board-panel board-panel-unified">
          ...
          {!isTemplateMode && stockActionModal ? (() => {
            if (stockActionModal.type === 'lecture') {
              ...
              const makeupBalanceForEntry = lectureEntry.studentId
                ? Math.max(0, makeupStockEntries.find((group) => group.studentId === lectureEntry.studentId)?.balance ?? 0)
                : 0
              ...
              return (
                <div className="auto-assign-modal-overlay" onClick={(event) => { if (event.target === event.currentTarget) setStockActionModal(null) }}>
                  <div className="auto-assign-modal" role="dialog" aria-modal="true" data-testid="stock-action-modal">
                    ...
                    <label className="auto-assign-include-makeup" data-testid="stock-action-modal-include-makeup">
                      <input
                        type="checkbox"
                        checked={includeMakeupInAutoAssign}
                        disabled={activeStockAutoAssignKey !== null}
                        onChange={(event) => setIncludeMakeupInAutoAssign(event.target.checked)}
                      />
                      <span>未消化振替も同時に自動割り当てする{makeupBalanceForEntry > 0 ? `（残${makeupBalanceForEntry}コマ）` : '（残なし）'}</span>
                    </label>
                    <div className="auto-assign-modal-actions">
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const includeMakeup = includeMakeupInAutoAssign
                          ...
                          await runStockAutoAssign(modalEntryKey, () => {
                            ...
                            setIsLectureStockOpen(false)
                            setIsMakeupStockOpen(false)
                            handleAutoAssignLectureStockEntry(lectureEntry, { includeMakeup })
            ...
            >
              ...
              {studentMenu?.mode === 'root' ? (
                <div className="student-menu-section">
                  ...
                  <button type="button" className="menu-link-button" onClick={handleOpenStudentEdit} data-testid="menu-edit-button">編集</button>
                      ...
                      <div className="student-menu-type-grid student-menu-type-grid-time">
                        {[
                          ...
                          <button
                            key={option.label}
                            type="button"
                            className={`student-type-button compact${normalizeRegularLessonNote(addExistingStudentDraft.noteSuffix) === option.value ? ' active' : ''}`}
                            onClick={() => setAddExistingStudentDraft((current) => (current ? { ...current, noteSuffix: option.value } : current))}
                            data-testid={`menu-add-lesson-minutes-${option.value || '90'}`}
                          >
                            {option.label}
                          </button>
                        ))}
                      ...
                      <div className="student-menu-type-grid student-menu-type-grid-time">
                        {[
                          ...
                          <button
                            key={option.label}
                            type="button"
                            className={`student-type-button compact${normalizeRegularLessonNote(editStudentDraft.noteSuffix) === option.value ? ' active' : ''}`}
                            onClick={() => setEditStudentDraft((current) => (current ? { ...current, noteSuffix: option.value } : current))}
                            data-testid={`menu-lesson-minutes-${option.value || '90'}`}
                          >
                            {option.label}
                          </button>
                        ))}
