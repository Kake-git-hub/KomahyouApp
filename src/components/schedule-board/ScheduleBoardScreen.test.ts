...
// 回帰防止(INV-05・2026-08-05): 生徒日程表の講習「希望数」を減らす正本は 2 系統あり、
...
describe('computeStudentMove (盤面移動の純粋ロジック・E2Eからの移植)', () => {
  ...
  // 「同コマに同一生徒が既にいる移動先はブロック」系(Issue #56 含む)は INV-12 マトリクス
  // (inv12-placement-uniqueness.matrix.test.ts)へ移設(2026-08-29・台帳新設に伴う昇格)。
...
describe('computeTeacherMove (講師の同コマ内移動/入れ替えの純粋ロジック)', () => {
  ...
  // 席まるごと入替(swapMode: 'seat'・2026-07-21 追加): 講師と生徒のペアを保ったまま席(desk.id)を交換する。
  it("swapMode='seat' は講師と生徒を一緒に入れ替え、ペアを保ったまま席番号を交換する", () => {
    const weeks: SlotCell[][] = [[
      mkCell('C1', [
        { id: 'd0', teacher: '田中', manualTeacher: false, teacherAssignmentTeacherId: 't_tanaka', lesson: mkLesson('la', [mkStudent('s1', '一郎'), null]) },
        { id: 'd1', teacher: '佐藤', manualTeacher: true, teacherAssignmentTeacherId: 't_sato', lesson: mkLesson('lb', [mkStudent('s2', '二郎'), null]) },
    ...
    const r = computeTeacherMove({ weeks, weekIndex: 0, cellId: 'C1', sourceDeskIndex: 0, targetDeskIndex: 1, swapMode: 'seat' })
    expect(r.status).toBe('moved')
    if (r.status !== 'moved') return
    expect(r.message).toContain('生徒ごと')
    const d0 = deskById(r.nextWeeks, 'C1', 'd0')
    const d1 = deskById(r.nextWeeks, 'C1', 'd1')
    // 席 d0 は佐藤+二郎、席 d1 は田中+一郎(講師と生徒のペアが保たれ席だけ入れ替わる)。
    expect(d0.teacher).toBe('佐藤')
    expect(d0.teacherAssignmentTeacherId).toBe('t_sato')
    expect(d0.lesson?.studentSlots[0]?.managedStudentId).toBe('s2')
    expect(d1.teacher).toBe('田中')
  ...
  it("swapMode='seat' で移動先が空席(講師も生徒もなし)なら講師と生徒がまとめて空席へ移る", () => {
    const weeks: SlotCell[][] = [[
      mkCell('C1', [
        { id: 'd0', teacher: '田中', manualTeacher: false, teacherAssignmentTeacherId: 't_tanaka', lesson: mkLesson('la', [mkStudent('s1', '一郎'), null]) },
        { id: 'd1', teacher: '', manualTeacher: false, lesson: undefined },
    ...
    const d0 = deskById(r.nextWeeks, 'C1', 'd0')
    const d1 = deskById(r.nextWeeks, 'C1', 'd1')
    // 元席は空に、空席側へ講師+生徒がまとめて移る。
    expect(d0.teacher).toBe('')
    expect(d0.lesson).toBeUndefined()
    expect(d1.teacher).toBe('田中')
    expect(d1.lesson?.studentSlots[0]?.managedStudentId).toBe('s1')
    expect(d1.manualTeacher).toBe(true) // 講師が入った机は manual 固定(再マージで消えない)
  })

  it("swapMode='teacher'(既定) は従来どおり講師だけ入替で生徒は各机に残る(回帰防止)", () => {
    const weeks: SlotCell[][] = [[
      mkCell('C1', [
        { id: 'd0', teacher: '田中', manualTeacher: true, teacherAssignmentTeacherId: 't_tanaka', lesson: mkLesson('la', [mkStudent('s1', '一郎'), null]) },
        { id: 'd1', teacher: '佐藤', manualTeacher: true, teacherAssignmentTeacherId: 't_sato', lesson: mkLesson('lb', [mkStudent('s2', '二郎'), null]) },
      ]),
    ]]
    const r = computeTeacherMove({ weeks, weekIndex: 0, cellId: 'C1', sourceDeskIndex: 0, targetDeskIndex: 1 })
    ...
    const d0 = deskById(r.nextWeeks, 'C1', 'd0')
    const d1 = deskById(r.nextWeeks, 'C1', 'd1')
    // 講師だけ入替、生徒は各机のまま(ペアが崩れる=teacher モードの意味)。
    expect(d0.teacher).toBe('佐藤')
    expect(d0.lesson?.studentSlots[0]?.managedStudentId).toBe('s1')
    expect(d1.teacher).toBe('田中')
    expect(d1.lesson?.studentSlots[0]?.managedStudentId).toBe('s2')
  ...
  it('teacherMoveInvolvesStudents: どちらかの机に生徒がいれば true / いなければ false', () => {
    const cellWithStudents = mkCell('C1', [
      { id: 'd0', teacher: '田中', lesson: mkLesson('la', [mkStudent('s1', '一郎'), null]) },
      { id: 'd1', teacher: '佐藤', lesson: undefined },
    ])
    expect(teacherMoveInvolvesStudents(cellWithStudents, 0, 1)).toBe(true)
    expect(teacherMoveInvolvesStudents(cellWithStudents, 1, 0)).toBe(true)

    const cellNoStudents = mkCell('C1', [
      { id: 'd0', teacher: '田中', lesson: undefined },
      { id: 'd1', teacher: '佐藤', lesson: undefined },
    ])
    expect(teacherMoveInvolvesStudents(cellNoStudents, 0, 1)).toBe(false)

    // 出席退避(lesson は空でも statusSlots に生徒名)の席も「生徒が絡む」と判定する。
    const cellWithStatusStudent = mkCell('C1', [
      { id: 'd0', teacher: '田中', lesson: undefined, statusSlots: [{ status: 'attended', name: '一郎' } as unknown as StudentStatusEntry, null] },
      { id: 'd1', teacher: '佐藤', lesson: undefined },
    ])
    expect(teacherMoveInvolvesStudents(cellWithStatusStudent, 0, 1)).toBe(true)

    // 同じ机同士 / cell 不在は false。
    expect(teacherMoveInvolvesStudents(cellWithStudents, 0, 0)).toBe(false)
    expect(teacherMoveInvolvesStudents(undefined, 0, 1)).toBe(false)
  })
})
