...
export function applyIOSSubmissionViewport(): void {
  if (typeof document === 'undefined') return
  if (!isIOS()) return
  if (!(IOS_VIEWPORT_WIDTH && IOS_VIEWPORT_WIDTH > 0)) return
  const meta = document.querySelector('meta[name="viewport"]')
  if (meta) meta.setAttribute('content', buildSubmissionViewportContent(IOS_VIEWPORT_WIDTH))
}
