...
- 日程表のセル（コマ）に表示するのは **actual（盤面の実際）だけ**。
  ...
  - ※ ただし回数表の比較用に planned 回数は算出する（後述 E）。
    ...
    - **【移行中】planned 通常回数（＝予定数）の算出方式**（オーナー確定 2026-08-05・正本は `docs/spec-invariants.md` INV-05）：
      - **旧方式（移行前）**：`expectedRegularOccurrences`（regularLessons＋テンプレ履歴から月次生成・
        参加者の在籍/進級/退塾の active 判定込み）が唯一の根拠。**盤面を見ない。**
      - **新方式（移行先）**：**その期間の実績 ＋ その期間の未振替の休み**（`computeOutstandingAbsenceOrigins`）。
        **盤面だけが根拠**で、出欠記録が無ければ実績と予定数は必ず一致する（＝警告が出ない）。
        期間外へ振替した分は元の期間の予定数からも減る。組み忘れ・組み過ぎは検出しない（仕様上の割り切り）。
      - 段階導入中（Step 2＝開発用教室のみ新方式）。**両方式の定義・移行段取り・割り切りは
        INV-05 に 1 回だけ書く**（重複ゼロ原則）。ここへ定義を複製しないこと。
...
- **印刷/配布時の出し分け（2026-07-04 監査領域9 B2 確定・オーナー確認済み）**：
  - 回数表の**警告スタンプ**・**通常回数の右括弧（予定数）**・**提出済バッジ**は**画面表示（室長確認用）のみ**で、印刷/配布時は非表示（`print-only-hidden`／`@media print { display:none }`）。
  - **講習回数の右括弧は印刷にも載せる**（2026-09-07 オーナー確定・Issue #61：講師が紙の実績だけを見て「講習終了」と誤読しカルテに記載した）。紙の見出しは **「講習回数(予定)」**（画面は「(希望数)」のまま）。右括弧は提出値そのものではなく「提出 ＋ 手動追加 − 削除」の予定総数なので、保護者向けには「希望」と書かない。
    - 印字するのは**提出由来の希望がある科目だけ**（`toCountRows` の `printDesired`）。希望が無く実績へフォールバックしている科目（表示期間に重なるセッションが無い紙・旧データの調整で 0 以下に潰れた行）と、**集団（集理/集社）**は従来どおり画面のみ。通常回数の予定数・講師日程表は印字しない。
    - ★既知の限界：希望数は**セッションの総数**、実績は**表示期間内**なので、同じセッションの講習を期間外（例：9月）へ置くと夏期の紙は「13(15)」で残2に見える（実際は9月に組み済み）。「残り」を紙に出すには盤面全体から数えた値を payload に渡す必要がある（未振替の休み `outstandingAbsences` と同型・未実装）。
