...
- **講習回数**：左＝actual（配置済み）／右括弧＝特別講習の登録希望数。不一致なら**講習回数表の直下に警告スタンプ**。
  - **科目名に授業時間（60/45分）を併記**（例 `英60分`）。90分（既定）は付けない（日程表セルと同ルール＝§D `formatScheduleMinutesSuffix`）。分数は**実配置コマの `noteSuffix` 由来**。科目内で 60/45 が一意なときのみ併記し、混在・不明・90分だけ・希望のみで未配置の科目には付けない（`pickLectureMinutesSuffix`＝誤解を招く併記をしない）。2026-07-06 実装（オーナー要望）。
