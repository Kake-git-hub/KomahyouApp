...
### E-1. 生徒日程表下部のオプション欄（休み欄置換・**全教室**）（2026-07-04 監査領域9 B4 確定・オーナー確認済み／2026-09-16 現状へ訂正）

- フィーチャーフラグ **`studentScheduleOptionField`**（`optionFieldEnabled` として payload に渡す・**全教室**＝`all-classrooms`。2026-06-27 のオーナー指示で開発用教室から昇格済みで、旧記述「開発用教室のみ」は現状と食い違っていたため 2026-09-16 に訂正）が有効なとき、生徒日程表下部の**休み欄（absence section）をオプション欄に置き換える**（振替を左詰めし、空きに**2列5行**のオプション欄を配置）。
...
- ⚠️ **現状の確定（2026-09-16 オーナー確定）**：全教室がこのレイアウトなので、**休み欄（absence section）は
  どの教室でも表示されない**（この下段レイアウト自体は今回の改定で変えない）。「振替待ちの休み」は
  **§E-1-1 の振替欄（元コマ起点・振替先が未配置なら「未定」）**と回数表の警告で確認する。
- なお App.tsx 同期経路は従来 `optionFieldEnabled` を欠いていた（上述 H-1 の payload 非対称・B1 と連動。dev-fix 起票済み）。

### E-1-1. 生徒日程表下部の振替欄（元コマ起点への統一・2026-09-16 オーナー確定）

振替欄（オプション欄レイアウトでは下段左・最低 7 行）は従来、**表示期間内に配置済みの振替コマ**＝振替**先**だけを
起点に「科目 元 → 先」を出していた。そのため **休んだ／別日へ動かした元コマが範囲内にあっても、振替先がまだ
組まれていない（未消化に残っている）と 1 行も出ない**＝室長が「振替を組み忘れている生徒」を日程表下部で
見つけられなかった。2026-09-16 のオーナー確定で**振替欄を元コマ起点に統一**する。

- **行の作り方**（機能フラグ `transferSourceRestDisplay` が ON のとき）
  1. **元起点の行**：表示期間内にあるその生徒の出欠記録 `absent`（**振替コマ自体の休みは除く**＝`makeupSourceDate`
     を持たない、または振替元日がそのセルの日付と同じもの）・`moved`・`holiday` から
     「**科目 元（M/D N限） → 先**」を作る。**先**の解決順は
     ① リンク先（`linkedDestinationDateKey` / `linkedDestinationSlotNumber`）があれば「M/D N限」
     ② 無く、その元コマ（生徒×科目×日付〈×限〉）が**未消化振替に残っていれば「未定」**
     ③ `moved` で移動先（`moveDestinationDateKey`）があればその日付・時限
     ④ それ以外は「未定」。
  2. **先起点の行（従来どおり）**：表示期間内に配置済みの振替コマ（`absent`／振無休でないもの）から「科目 元 → 先」。
  3. 1 と 2 を**元コマ（日付＋時限＋科目）で重複排除**し、**元コマの日付昇順**で並べる。
- ★**「未定」の根拠は未消化振替一覧の正本**（`buildMakeupStockEntries` の `remainingOriginDates` /
  `remainingOriginSlots`）を payload（`outstandingMakeupOrigins`）へ載せて使う。**日程表側で在庫を数え直さない**
  （数え直すと盤面の残数と食い違う＝INV-06 の温床）。生徒の同一性は既存の `buildOutstandingAbsenceCountMap` と
  同じ規則（生徒 ID → 表示名の順）。
- **表示範囲の規則**：振替元・振替先の**どちらかが表示期間内にあれば出す**（元だけ範囲内＝「→ 先」または
  「→ 未定」／先だけ範囲内＝従来どおり）。**どちらも範囲外の行は出さない**。
- 講習（`special`）の記録は振替欄に出さない（従来どおり）。書式は従来の圧縮規則
  （`compactMakeupSourceLabel` / `compactMakeupDateSlot`・文字数超過で縮小・最低 7 行）をそのまま使う。
  「未定」は右側の文字列としてそのまま出す。
- **フラグ OFF のときは従来（先起点のみ）の振替欄**にする（下段レイアウト・欄の幅・行数は ON/OFF とも不変）。
- ⚠️ **消してはならないガード**：`moved` / `holiday` の出欠記録が payload（`serializeCells` の `statuses`）に
  載るのは**この改定が初めて**（従来は `serializeCells` が `moved` を捨てていた）。**集計系からは必ず除外**すること。
  除外先＝講師日程表のセル表示・講師の振替メモ・**交通費の日数／給与のコマ数／講師回数（§E-2）**、
  生徒日程表の**通常回数（実績）・講習回数**、出欠入力・QR の「出欠済み」判定。混ざると
  **授業をしていない日が実績・給与に載る**（INV-05／INV-06 の症状）。
- 生徒日程表のセル表示では `moved` / `holiday` を `absent` と同じ形（「休 M/D」・補足は「科目 / 種別」）で描く。
  リンク先が無い `moved` は移動先（`moveDestinationDateKey`）の日付を出す。休日列（`is-holiday`）でも描く。
  カードを掴めない（ドラッグ不可）ことは `spec-student-schedule-dnd.md` §C-1。
- 記録種別と表示ラベルの正本は `spec-makeup-stock.md` §3-1、`holiday` の会計ガード一覧は
  `spec-invariants.md` INV-06（ここへ複製しない）。

### E-2. 講師日程表の給与計算欄（2026-09-13 オーナー確定）

- **コマ数（A/B/C/D・集団）**：出席（attended）の記録だけを数える（集団は出席1名以上で1コマ）。
- **交通費の日数**：その講師に**出席記録が1件以上ある日**＋**集団授業を実施した日**（集団だけの日も1日・spec-group-lesson §F）。予定だけで出欠を付けていない日・生徒が全員休みの日は数えない（2026-09-13 に「記録ベースに広げる」案を検討し、バグではないため**現行維持**で確定）。日程表は開いた時／「最新表示」でしか再計算しないので、期間の途中で開いた画面・印刷は当時の出席記録での値になる。
- **小計の手入力**：小計欄は直接入力でき、入力した値を**正**として合計に反映する（特別な事情の調整用）。
  - 小計が空欄なら「単価×コマ数（交通費は日数）」、事務給は単価そのものを小計欄に表示する。
  - 小計を空にすると自動計算に戻る。単価を変えても手入力した小計は上書きしない。
  - 手入力した小計は画面だけ薄い黄色で見分ける（印刷では色なし）。
  - 手入力の小計は**端末に記憶しない**（開き直す・「最新表示」で自動計算に戻る）。単価は従来どおり講師ごとに端末へ記憶する。
  - 「12,000」「12000円」の表記も数値として受け、数値として読めない入力は 0 として合計する。
...
- 「PDF出力」を押すと**コマ選択モーダル**（`BoardPrintSelectionModal`）が開く。行＝時限・列＝**表示週の全曜日**のチェック表。
  **定休日（`isOpenDay=false`）も選択できる**（薄く表示・盤面には空の列として出る。確認リスト p-1・オーナー指示 2026-09-12「日曜も候補に」。
  それ以前は選択対象外だったが「日曜だけ出す」ができなかったため改めた）。行頭（時限）・列頭（曜日）で一括トグル、全選択／全解除ボタンつき。
- **初期状態は「その教室で前回出力した選択」**（曜日×時限のパターンを `localStorage` の `board-print-selection:<教室>` に記憶し、
  週が変わっても同じ曜日×時限に当てる。記憶が無い／壊れている／当たるセルが無いときは全選択）。確認リスト p-1（2026-09-12）。
  **全選択＝これまでの「1週間まるごと」出力と同一**（間引きを一切しない）。⚠️ この「全選択＝現状と同一出力」は
  ...
  例外として 2026-09-12 に全経路共通で直した見切れ 2 点（集団行の時限ラベル「集団」を横書き化・講師名を列幅に収まるまで縮める）は
  全選択にも効く（見切れていたケースだけが変わる）。
- **空選択は出力不可**（「出力」ボタンを無効化）。
...
- **用紙は A3 縦固定のまま**（オーナー確定 2026-09-11。A4 化・縦横自動選択はしない）。間引き後の表を A3 縦いっぱいに使う倍率は
  `resolveBoardPrintLayoutRelief`（2026-09-12・確認リスト p-2/p-3）で決める：表が紙より**縦長（曜日を絞った）なら列を横に伸ばす**
  （従来の幅合わせ）、**横長（時限を絞った）なら行を縦に伸ばす**（`rowScale`・上限 4。従来は幅基準で縮小され下半分が白紙だった）。
  文字は `fontRelief`（上限 3）で上限を緩める：生徒 34px→最大 72px（`resolveBoardPrintStudentMaxFontSizeByRelief`）、講師名 24px→relief 倍、
  席番号 22px→列が伸びるときだけ relief 倍。**講師名は列幅に収まるまで縮める**（`fitSingleLineTextForPdf`・従来は 24px 固定で 3 文字が見切れた）。
  合わせて `html2canvas` の解像度を残る矩形サイズに応じて上げる（`resolveBoardPrintCanvasScale`・上限 3・**全選択は従来の 1.1 固定**）。
  **全選択は倍率 {1,1,1}（34px / 24px / 22px のまま）**。はみ出し判定は従来どおり効く。
  **机行の高さは全行同じ**（2026-09-12・確認リスト第2版 p-2/p-3・Issue #63）：部分選択では生徒欄の内側（`.sa-student-inner`）を
  「机行の高さ（62px × rowScale）− 上下 padding」に固定し（`lockStudentInnerHeightForPdf`）、生徒文字はその高さに収まるまで縮める。
  従来は緩めた上限（最大 72px）まで大きくなった生徒名 2 行が行を押し広げ、生徒のいる行だけ高く・空席の行は低いと不揃いになっていた。
  全選択では固定しない（従来出力と同一）。
  **固定した内側のはみ出し判定は「子の高さの合計 vs 固定高さ」で行う**（2026-09-13・確認リスト第3版 p-2/p-3・`studentInnerContentOverflows`）：
  内側は flex(column) なので高さを固定すると子（名前行・学年科目）が flex-shrink で押し潰され、`scrollHeight` が伸びず
  はみ出しを検出できなかった（＝上限 72px のまま描かれ名前と学年科目が重なった）。子の `flex-shrink` を 0 にし、目印属性
  `data-pdf-locked-height` の付いた内側だけ合計高さで判定する。全選択（固定しない）は従来判定のまま。
  **講師名（1 行テキスト）のはみ出し判定は「文字幅 vs ボックス自身の幅」で行う**（2026-09-13・確認リスト第4版 p-3・`singleLineTextOverflows`）：
  従来は td（padding 込み）の幅と scrollWidth を比べていたため、中央寄せ flex の左側はみ出しが scrollWidth に載らず
  「収まった」と誤判定していた（実測: 文字幅 65px・ボックス 50px でも scrollWidth 57 ≤ 56+1）。Range の矩形で文字幅を測り、
  講師名ボックスの clientWidth と比べる。
- **集団行の未設定科目セルの「＋ 科目を選択」ガイドは PDF に出さない**（`clearGroupSubjectPlaceholdersForPdf`・2026-09-13・第4版 p-3 メモ
  「未割り当ては空白でよい」）。画面の CSS ::before による操作ガイドで、紙には意味が無い。全選択・部分選択とも空白にする（クローンのみ・盤面 DOM は不変）。
- **出力中は画面全体に「PDF出力中…」のスピナー**（`BusyOverlay`・2026-09-12・確認リスト v1.5.504 その他）。出力が終わるまで
  下の盤面は操作できない。テンプレ上書き保存のレポート PDF も同じ。
- **PDF のタイトル（＝ファイル名）は選択に応じて組む**（`buildBoardPrintTitle`・2026-09-12・p-2）：全選択は従来の週タイトル
  （`週間予定表9月14日-9月20日`）、曜日を絞ると `予定表9月14日(月)-9月16日(水)`（連続）／`予定表9月14日(月)・9月16日(水)`（とびとび）、
  時限を絞ると末尾に ` 1限-3限`／` 1限・3限` を添える。
- 集団行の時限ラベル「集団」は**横書き**（2026-09-12・p-6。通常の時限ラベルと同じ縦回転 36px だと 40px の行で見切れた）。
- 機能フラグ `boardPrintSelection`（`featureRollout.ts`）。**OFF の教室は従来どおりモーダルなしで即出力**する。開発用教室で先行検証（v1.5.500〜509）したうえで **全教室へ昇格済み（オーナー指示 2026-09-13）**。回帰で development-only へ戻さない。
