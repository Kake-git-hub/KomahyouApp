...
## 進捗（2026-06-12 実装セッション）

| ID | 状態 | 反映バージョン | 備考 |
|----|------|----------------|------|
| B3 | ✅ 完了・デプロイ済 | 1.5.303 | QR提出の休日反映・提出済み閲覧・初回通知抑止。回帰テスト追加 |
| A2 | ✅ 完了・デプロイ済 | 1.5.303 | タブを隠した時の無変更ガード。`shouldSyncWorkspaceOnVisibilityHidden` |
| A1 | ✅ 完了・デプロイ済 | 1.5.304 | サーバー版数照合(楽観ロック)。functions先行→client段階デプロイ。後方互換 |
| A3 | ✅ 完了・デプロイ済 | rules 1.5.304時 | classroomSnapshots/Settings 直書きを開発者のみに。マネージャーはCF経由で影響なし |
| C1 | ✅ 完了 | 1.5.305(予定) | 配布/開発者/請求/バックアップ復元画面を遅延読み込み。Appチャンク縮小 |
| B2/A4 | ✅ 完了 | 1.5.305(予定) | 死蔵 appSnapshotSync.ts 削除・cleanupOldSaveAttempts(30日保持の定期削除)追加 |

すべての改修項目(B3/A2/A1/A3/C1/B2/A4)が完了。残るは長期課題の C2(巨大ファイルの段階分割)のみ。

> **A1の推奨追加確認（オーナー向け）**：本番反映済みだが、念のため**開発用教室で1回保存**して
> 「保存→Firebase同期しました」が出ることを確認すると安心（後方互換設計＋テスト済みのため理論上は問題なし）。
> 別端末から同教室を開いて編集→保存すると、片方で「別の端末で更新されました」バナーが出れば正常動作。
