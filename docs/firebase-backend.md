...
- データ: Firestore
- 管理系 API: Firebase Functions
- サーバーバックアップ: Cloud Storage

Hosting は `firebase.json` で `dist` をそのまま配信し、SPA なので全パスを `index.html` へ rewrite します。Spark 無料プランでは既存教室の閲覧・編集・snapshot 同期だけをアプリ側で行い、Auth を伴う管理系操作とサーバー自動バックアップは Blaze へ移行したときだけ有効化します。
...
### `workspaces/{workspaceKey}/workspaceAutoBackupSummaries/{backupDateKey}`

- Blaze + Functions + Cloud Storage を有効にした場合の日次サーバーバックアップ要約
- フィールド
  - `backupDateKey`
  - `savedAt`
  - `sourceSavedAt`
...
## Blaze でのサーバー自動バックアップ

- Functions の scheduled trigger が毎日 JST 02:10 に全 `workspace` を走査します
- 各ワークスペースの現行 Firestore 状態を、開発者バックアップ互換の JSON として Cloud Storage の `workspace-auto-backups/{workspaceKey}/{backupDateKey}.json` へ保存します
- Firestore には `workspaceAutoBackupSummaries/{backupDateKey}` だけを保存し、保持期間は直近 14 日です
- `functions/.env` に `GOOGLE_DRIVE_BACKUP_FOLDER_ID` を設定すると、同じサーバーバックアップ JSON を Google Drive にも自動同期します
- Google Drive サーバー同期は Web アプリを開かなくても動作します。共有ドライブの場合は Drive folder を Cloud Functions の実行サービスアカウントへ共有するか、`GOOGLE_DRIVE_SERVICE_ACCOUNT_JSON_BASE64` に base64 化したサービスアカウント JSON を設定してください
- 通常のマイドライブフォルダへ保存する場合、サービスアカウントには Drive 保存容量がないため `storageQuotaExceeded` になります。この場合は `GOOGLE_DRIVE_OAUTH_CLIENT_ID` / `GOOGLE_DRIVE_OAUTH_CLIENT_SECRET` / `GOOGLE_DRIVE_OAUTH_REFRESH_TOKEN` を設定して、フォルダ所有者の OAuth refresh token で同期してください
- Google Drive 側も Cloud Storage と同じく daily 14 本 / hourly 72 時間ぶんに合わせて古いバックアップを削除します
- 開発者画面の `今すぐバックアップ実行` は Google Drive 同期の診断も表示します。同期失敗時は、Drive API 未有効、フォルダ ID、共有先サービスアカウント、権限不足などの原因を画面で確認できます
- 現在の開発者画面バックアップ一覧はブラウザ内自動バックアップを表示しており、このサーバーバックアップ一覧とはまだ統合していません
- この機能を使うには Blaze プラン、Cloud Storage、有効な Functions デプロイが必要です

### Google Drive サーバー同期の設定

1. Google Cloud project で Google Drive API (`drive.googleapis.com`) を有効化する
2. バックアップ保存先の Google Drive folder を用意する
3. その folder を Cloud Functions の実行サービスアカウントへ共有する
4. `functions/.env` に少なくとも次を設定する

```bash
GOOGLE_DRIVE_BACKUP_FOLDER_ID=
```

明示サービスアカウントを使う場合は次も設定します。

```bash
GOOGLE_DRIVE_SERVICE_ACCOUNT_JSON_BASE64=
```

`GOOGLE_DRIVE_SERVICE_ACCOUNT_JSON_BASE64` には、サービスアカウント JSON 全体を base64 化した 1 行文字列を入れてください。

通常のマイドライブへ保存する場合は、サービスアカウントではなく OAuth refresh token を使います。
...
通常のマイドライブ folder では、Cloud Functions のサービスアカウントは `storageQuotaExceeded` で新規ファイルを作れません。サービスアカウントのまま運用する場合は、保存先を共有ドライブへ移してください。

前提:

- 共有ドライブは Google Workspace 環境で利用します。個人の Google アカウントだけでは共有ドライブを作れない場合があります
- サービスアカウントは共有ドライブのメンバーとして追加します。Google Drive API の共有ドライブ運用では、サービスアカウントは共有ドライブ内へアップロードする形が前提です

手順:

1. Google Drive を開き、左メニューの `共有ドライブ` から新しい共有ドライブを作る
2. その共有ドライブの中にバックアップ保存用 folder を作る
3. 共有ドライブ右上の `メンバーを管理` または `メンバーを追加` から、Cloud Functions の実行サービスアカウント `76394427193-compute@developer.gserviceaccount.com` を追加する
4. 権限は少なくとも `コンテンツ管理者` か `管理者` にする
5. 作成した backup folder を開き、URL から folder ID を取得する
6. `functions/.env` の `GOOGLE_DRIVE_BACKUP_FOLDER_ID` をその共有ドライブ内 folder の ID に差し替える
7. Functions を再デプロイする

```bash
npx firebase-tools deploy --project komahyouapp-prod --only functions:createWorkspaceServerAutoBackups,functions:createWorkspaceServerHourlyBackups,functions:triggerWorkspaceServerAutoBackup
```

8. 開発者画面の `今すぐバックアップ実行` を押し、Google Drive 診断が `成功` になることを確認する

確認ポイント:

- 診断の認証が `Functions 実行サービスアカウント` になっていること
- サービスアカウント email が `76394427193-compute@developer.gserviceaccount.com` であること
- エラーが `storageQuotaExceeded` から `成功` に変わること

共有ドライブが使えない場合は、共有設定ではなく OAuth refresh token 方式へ切り替える必要があります。

### マイドライブで運用する OAuth refresh token 手順

共有ドライブが使えない場合は、Drive の所有者本人の OAuth refresh token で保存します。コード側はすでに対応済みなので、残りは Google Cloud 側の OAuth client 作成と token 発行だけです。

手順:

1. Google Cloud Console で OAuth 同意画面を作成する
2. Google Cloud Console の `認証情報` で `OAuth クライアント ID` を新規作成し、種類は `デスクトップ アプリ` を選ぶ
3. `functions/.env` に次を入れる
...
4. 次のコマンドでローカル helper を起動する

```bash
npm --prefix functions run drive:oauth-token
```

5. ブラウザで Google 認証を完了すると、helper が `GOOGLE_DRIVE_OAUTH_REFRESH_TOKEN=` を表示する
6. その値を `functions/.env` に設定する。helper に `--write-env` を付けるとローカル `functions/.env` を自動更新できる
...
8. 開発者画面の `今すぐバックアップ実行` を押し、Google Drive 診断が `認証: Google Drive OAuth` / `状態: 成功` になることを確認する

注意:

- Google の OAuth ドキュメント上、Windows デスクトップアプリは loopback redirect で認証コードを受け取れます
- OAuth 同意画面が `External` かつ `Testing` のままだと、refresh token は 7 日で失効する場合があります。長期運用するなら `Production` にするか、Workspace の `Internal` を使ってください
- 既に同じ client/user 組み合わせで token を持っていて refresh token が返らない場合は、Google アカウントのアプリ権限を一度削除してから再実行してください
...
`deploy:firebase:with-functions` は Hosting / Firestore / Functions をまとめてデプロイします。Spark のままでは scheduled Functions が動かないため、サーバー自動バックアップは実行されません。
