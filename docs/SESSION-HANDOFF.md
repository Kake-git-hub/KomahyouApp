...
**Phase 4（振替 A/B/C）は完了・検証済み・2026-06-09 にデプロイ（v1.5.290）。** 本番＝main 同期。
オーナーは現在「**別件の修正**」に着手中（このセッションの続きではない）。

→ **再開時の次タスク候補**：
1. **テンプレ挙動の仕様確定**：`docs/spec-template-behavior.md`（テンプレ全挙動の確認用 質問群Q1-Q20・**オーナー未回答**）に回答 → 正本化。特に Q4/Q5（休日/定休日との力関係）・Q11（何年先まで生成）・Q9（手動振替の非復元）が要注意。
...
- ~~**Phase 0〜4**~~ → ✅ 完了・**v1.5.290 デプロイ済**（§3）。
- **テンプレ仕様確定**：`docs/spec-template-behavior.md`（Q1-Q20・**オーナー未回答**）に回答 → 正本化。
