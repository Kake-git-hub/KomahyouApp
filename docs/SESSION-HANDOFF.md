...
13. **board-sync-publish の全週clone を週単位キャッシュ化（1.5.283）**:
    - **1.5.282 実測**: 起動 660→**118MB**、編集ピーク 1466→**653MB**。日程表同期はピーク要因から外れた
      (653MBの t=60 は board-render:9 / board-sync-publish / overlay-recompute / boardshare-publish。
      student/teacher-schedule-sync は出ていない)。残る最大レバーは**編集ごとの publish 全週clone**。
    - **安全監査**: App は `boardState.weeks` を**読み取りのみ**(trim/署名/配布/popup参照=非破壊)、盤面側も
      normalizedWeeks は描画専用で非破壊。published 週は双方とも変更されないことを確認。
    - **対策**: `cloneWeeksForPublish`(WeakMap, 入力週参照キー)を新設。`applyClassroomAvailability` の出力週は
      メモ化で未変更週の参照が安定するため、その参照で deep clone をキャッシュし**未変更週は前回クローンを再利用**。
      publish 3箇所(overlay/board-sync-publish/commitWeeks)を置換。`let nextWeeks = cloneWeeks(...)`(編集用可変)
      は据え置き。→ 1編集の publish clone が「変更週だけ」に限定。
    - **検証**: 型/unit 239件(+2) green。ローカルで「現在週/翌週それぞれの出席編集が正しく当該セルに反映/Undo復元/
      コンソールエラーなし」を確認(キャッシュが別週の古い内容を返さないこと=変更週は別参照→別クローン)。
    - **次段(まだ高い場合)**: autosave の頻度/差分化(毎編集で全ワークスペースを IndexedDB へ書込)、board-render 回数削減、
      最新表示の App+Board 二重同期 dedup。

14. **autosave の頻度低減（1.5.284）**:
    - **1.5.283 実測**: アイドル **170MB** / 編集ピーク **~495MB** / 編集後 ~240MB に戻る健全なノコギリ波。
      1.5GB暴走は解消済み。残コストは autosave(毎編集で workspace 全体を IndexedDB へ structuredClone)。
    - **対策**: autosave の debounce を 250ms → **2000ms**、ただし前回保存から **最大15秒(maxWait)で必ず保存**。
      `autosaveLastStartedAtRef` で経過を見て `delay = clamp(0, min(2000, 15000 - 経過))`。連続入力中の全直列化回数を
      大幅削減。タブ切替/最小化/クローズは既存の `visibilitychange`(hidden)/`beforeunload` で flush されるため
      通常離脱で消失なし(クラッシュ時の未保存幅のみ最大~15秒に拡大=ユーザー了承済み)。
    - **差分保存(変更教室だけ)は今回見送り**: IndexedDB は workspace 全体を1レコード(`store.put(snapshot, KEY)`)で
      保存しており、教室別レコード化は読込/リモート同期/マイグレーションに波及しデータ消失リスクが高い。別途設計が必要。
    - 注: **autosave は `navigator.webdriver` かつ firebase 無効時 `isSnapshotPersistenceRuntimeEnabled()=false` で
      無効化される**ため、Preview の自動操作(local)では観測不可。型/unit 239件 green＋編集/Undo回帰のみ確認。
      本番 ?memlog で autosave-run 回数の減少と編集ピーク低下を要確認。
