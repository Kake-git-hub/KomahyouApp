...
### ⚠️ 未完の最優先タスク（オーナー操作待ち・私〔Claude〕は本番教室へ書込不可）
- **日大前校（`5w5OMueETerSKrSf14HC`）がまだ未復旧**。本物データ＝生徒129名・テンプレ35セル(eff 2026-09-07, stuHash `0c691a4075`)。現状の日大前スロットは旧・空の開発用データ(生徒73名・テンプレ無, `d19bb93b16`)で上書きされたまま。
  - **復旧手順**：各PCで Ctrl+Shift+R（v1.5.301を読込）→ 開発者画面 → サーバーバックアップ →「**2026-06-11 07:10 毎時**」→ この時点へ復元 → **日大前校だけ選択** → 保存。
  - 補足：日大前の本物データは現在 **開発用教室スロット**(`v8OZ7zH8vONNHjjYVcR1`, 生徒129)にも入っている。復旧後、開発用教室は Feature B（下記）で任意の正しい時点を読み込んで整え直せる。
  - 新セッションは**まず読み取り専用で現状を再確認**（gcloud token で Storage `workspace-auto-backups/main/hourly/{key}.json` を GET、各教室の stuHash＝生徒IDソートのsha1先頭10桁・テンプレ cell数/eff を照合）。手法は memory `komahyou-classroom-restore-cross-contamination.md` に記録済。
- **全端末でハードリロード徹底**：今回の混入の最大の引き金は「旧バンドルのキャッシュのまま操作」。各PCで一度 Ctrl+Shift+R。
...
3. **最優先：0-A の日大前校 復旧がオーナー側で完了したか確認**（読み取り専用で各教室 stuHash を照合。日大前が `0c691a4075`/生徒129/テンプレ35 に戻っていればOK）。未完ならオーナーに復旧手順を案内（私は本番教室へ書込不可）。
4. 復旧確認後の持ち越し候補：
   - localStorage キーのアカウント別分離（混入の最後の残リスク・0-A 参照）。
   - 開発用教室のクリーンアップ（現状 日大前由来データ＋テスト残差。Feature B で正しい時点を読み込み直せる）。
   - ロードマップ系（下記 §0/§6）：⑧の残オーナー判断、多ルール×全員のパフォーマンス最適化、`spec-template-behavior.md` Q1-Q20 確定。
