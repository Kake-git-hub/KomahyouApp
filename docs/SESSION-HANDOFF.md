...
- **⑧ 自動割振の「オーナー判断」2件：現状は実害なし（どの教室もルール未設定のため）。**
  - 「区分=制約のハードフィルタ化」：今は全ルールがソフト(スコア＋警告)。ルールを実際に使い始めるまで挙動差は出ない＝**今は問題なし**。実運用でルールを使う段になったら「制約は絶対禁止にするか」を決める、という保留事項。
  - 「旧『2限寄り/5限寄り』設定教室の自動移行」：そんな旧設定を持つ教室が**存在しない**ため移行対象ゼロ＝**問題なし**。
  - ⇒ ⑧はバグ・宿題ではなく「使い始めたら判断」メモ。次セッションで能動対応は不要。
- **【引き継ぎ：要対応候補】多ルール×「全員」対象のパフォーマンス最適化**：詳細は §0 ⑧ の「【課題】」参照（6ルール×全員で割振・警告再計算が数十秒ブロックした実機事例）。実運用で多ルールを使う教室が出たら、対象判定のメモ化（studentId→適用可否キャッシュ）/警告計算の分割で対処。
...
4. 持ち越し候補（いずれも能動着手はオーナー指示後。優先度順）：
   - **多ルール×「全員」対象のパフォーマンス最適化**（実運用で多ルール教室が出たら。§0-A／§0 ⑧「【課題】」）。
   - localStorage キーのアカウント別分離（混入の最後の残リスク・§0-A）。
   - 開発用教室のクリーンアップ（日大前由来データ＋⑧テスト残差。Feature B で正しい時点を読み込み直せる）。
   - **テンプレ Q1-Q20＝確定＋実装済み（追加作業なし）／⑧オーナー判断2件＝ルール未使用ゆえ今は実害なし（§0-A）。** ＝当面の能動タスクは無し。新規はオーナー指示待ち。
