...
> ⚠️ **2026-06-08 の全体見直しで一部仕様を変更。** 新しい正本は `docs/spec-schedule-pdf.md`。
> 特に「planned の基準（テンプレート由来）」「色分け撤廃（ラベル表記で区別）」「登録の日程表一本化」は本書ではなく新正本を優先する。
...
> 括弧内の呼称は出どころで呼び分ける（正本＝`docs/spec-invariants.md` INV-05「呼称の正本」）。
> 通常＝テンプレ由来の**予定数**、講習＝提出由来の**希望数**。両方を「希望数」と書かない。
