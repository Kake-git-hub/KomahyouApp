...
> **確定仕様は `docs/spec-multi-tenant.md`（Phase 0 起案 2026-09-16）**。本書と食い違う場合はそちらが正本。
...
> **状況（2026-09-18）**: T1-1〜T1-5 実装済み（既定値で出力不変）。詳細は `docs/spec-multi-tenant.md` §11。
> **Phase 2〜3 の実行順は `docs/plan-2026-09-18-second-company-onboarding.md` で組み替え**（フォークは切らず、会社別プロファイル＋会社別ビルドで受け入れる。既存運営会社＝株式会社アーチ）。
