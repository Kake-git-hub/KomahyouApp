...
- **会社 ＝ `workspaces/{会社キー}`**。既存 3 教室は `main` のまま（= スクールIE 社。改名移行はしない。§9-2）。
...
| T0-2 | 開発用教室判定の是正: 名前一致を廃止し **`{workspaceKey, classroomId}` の組**で判定。クライアント／サーバーを 1 つの述語モジュールに寄せ、パリティテストで固定（6-2, 6-3） | dev-fix（Opus） | `developmentClassroom.test.ts` ＋ functions 側パリティ | 1 |
...
| T1-2 | 機能フラグ 3 段化: `isFeatureEnabledForClassroom(key, classroom)` を **scope → 会社既定 → 教室上書き**に。O-1（`classrooms/{id}.featureOverrides`・配線 7 箇所）をここで実施（6-4）。サーバー側述語も同じ順序 | dev-fix（Opus） | `featureRollout.test.ts` 拡張、CF 入力検証、`test:rules` | 1.5 |
...
| T3-3 | 横断請求（§5）: 会社合算請求書、全社一覧、`listBillingWorkspaces`、許可者の member 化（6-5） | dev-fix（Opus） | `billing.test.ts`、CF 検証、`test:rules` | 1.5 |
...
### 9-1. リポジトリ方式
- [ ] **B: 上流フォーク**（推奨）／ A: 単一リポ＋会社プロファイル／ C: コアのパッケージ化: ______
- [ ] Phase 1 完了時点で「フォークを切るかは 2 社目の要望を見て決める」段階運用でよいか（推奨: はい）: ______

### 9-2. 既存 3 教室の扱い
- [ ] `workspaces/main` をそのまま「スクールIE 社」の workspace とし、改名移行はしない（推奨: はい）: ______

### 9-3. 会社ごとの URL
- [ ] 同一プロジェクトの multi-site（`komahyou-<会社>.web.app`）で始め、必要な会社だけカスタムドメイン（推奨）: ______

### 9-4. 一括請求の形
- [ ] 会社（workspace）宛の**合算請求書**を追加し、教室宛は会社の希望で選べる（推奨）／ 教室宛のまま全社一覧で一括作成のみ: ______
- [ ] 請求許可者をメールのハードコードから member のフラグへ移す（推奨: はい）: ______

### 9-5. 呼称
- [ ] 「室長」など役割名を会社ごとに変えられるようにするか（推奨: 役割名 2〜3 語だけ。全域は要望が出てから）: ______

### 9-6. 開発用教室
- [ ] 会社ごとに開発用教室を持つ（推奨: 持つ。会社固有機能の検証先が要る）／ 開発用教室はコア（main）のみ: ______

### 9-7. 他社の開発担当者
- [ ] 他社側の開発担当を developer として入れる想定はあるか（あるなら当該 workspace のみ付与・コアリポは非公開のまま）: ______

### 9-8. 別プロジェクト分離の例外
- [ ] 規約上プロジェクト分離を要求する会社が出た場合だけ、例外として別プロジェクト（コアの env 切替で対応）: ______

### 9-9. 着手順
- [ ] Phase 0 → 1 の順にコアリポ内で先に進める（2 社目未確定でも着手。推奨: はい）: ______
- [ ] O-1（教室別オプション基盤・保留中）を Phase 1 の T1-2 として再開してよいか: ______
