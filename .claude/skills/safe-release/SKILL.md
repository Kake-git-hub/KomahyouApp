...
1. **自動テストが網羅的に通る** — マージ前にローカル/CI で unit + e2e を緑にする
...
```bash
npm run lint
npm run test:unit          # vitest（回帰防止テストを含む）
npm run test:e2e           # Playwright（local モード）
npm run test:e2e:firebase  # Firebase エミュレータ e2e（任意・保存系を触ったとき）
...
- [ ] lint / test:unit / test:e2e / build が緑
