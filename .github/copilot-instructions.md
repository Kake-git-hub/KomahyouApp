...
- Current implementation notes
	...
	- `RegularLessonTemplateEditor` must show only participants visible in management data at `effectiveStartDate`: teachers are limited to `resolveTeacherRosterStatus(...) === '在籍'`, students exclude only `退塾` and `非表示`. Preview/export/save must sanitize the template with the same rule.
