...
- Current implementation notes
	...
	- `buildManagedRegularLessonsRange` must place teachers regardless of `resolveTeacherRosterStatus` result. Templates represent the intended schedule; roster status (入塾前/退塾) must not remove teachers from managed placement. This avoids orphaned student-only desks.
	- Template participant filtering must follow `filterTemplateParticipantsForReferenceDate` (spec-template-behavior Q14: exclude only `退塾`/`非表示`; pre-enrollment participants may be placed ahead of time). The modal `RegularLessonTemplateEditor.tsx` was dead code and deleted on 2026-07-04; the only template editing path is the onboard `templateCells` mode in `ScheduleBoardScreen`.
	- Template overwrite must NOT restore `manualMakeupAdjustments` for post-freeze origins. All suppressions >= effectiveStart are cleared and regular lessons are rebuilt from the new template. Restoring manual adjustments causes double-counting (board lesson + stock entry).
